...
void test_virtualization_queries() {
    ...
    expect(sandboxed.has_value() ||
           ...
           "sandbox presence query must succeed or report not_supported");

    const auto sandbox = syscape::virtualization::sandbox();
    expect(sandbox.has_value() ||
               sandbox.error() == syscape::errc::not_supported,
           "sandbox classification query must succeed or report not_supported");
}
