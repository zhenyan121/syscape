...
void test_filesystem_queries() {
    ...
    expect(
        mounts.has_value() || mounts.error() == syscape::errc::not_found ||
            mounts.error() == syscape::errc::not_supported,
        "mounts query must succeed, report not_found, or report not_supported");
