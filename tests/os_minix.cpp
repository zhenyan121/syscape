...
void test_parsing_functions() {
    ...
    // Hostname parsing
    {
        const auto res = parse_host_name("minixbox");
        expect(res.has_value() && *res == "minixbox",
               "valid hostname 'minixbox' must parse");
    }
    {
    ...
    {
        const auto res = parse_version_string("3.4.0\n");
        expect(res.has_value() && *res == "3.4.0",
               "version '3.4.0' must parse and trim newline");
    }
    {
        const auto res = parse_version_string("  3.3.0RC6  ");
        expect(res.has_value() && *res == "3.3.0RC6",
...
void test_runtime_queries() {
    expect(syscape::target_operating_system() ==
               syscape::operating_system::minix,
           "target_operating_system must report minix");
    ...
    expect(build.error() == syscape::errc::not_supported,
           "build identifier must report not_supported on MINIX");

    const auto boot_id = syscape::os::boot_identifier();
    expect(boot_id.error() == syscape::errc::not_supported,
           "boot identifier must report not_supported on MINIX");
}
