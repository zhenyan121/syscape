...
void test_dns_parsing() {
    ...
    const auto extra_server =
        syscape::detail::network_backend::parse_dns_content(
            "nameserver 1.1.1.1 unexpected\n");
    expect(!extra_server &&
               extra_server.error() == syscape::errc::malformed_data,
           "nameserver with extra tokens must report malformed_data");
    ...
    expect(!empty_search &&
           ...
           "empty search directive must report malformed_data");

    const auto extra_domain =
        syscape::detail::network_backend::parse_dns_content(
            "domain example.com unexpected\n");
    expect(!extra_domain &&
               extra_domain.error() == syscape::errc::malformed_data,
           "domain with extra tokens must report malformed_data");
}
...
void test_network_queries() {
    ...
    expect(stats.error() == syscape::errc::not_supported,
           "statistics query must report not_supported on RTEMS");
}
