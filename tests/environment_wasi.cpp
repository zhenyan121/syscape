...
void test_environment_queries() {
    ...
    const auto missing =
        syscape::environment::get("SYSCAPE_NONEXISTENT_VAR_WASI");
    ...
    const std::string test_file =
        "/tmp/syscape_wasi_not_a_dir_" + std::to_string(::getpid());
    ...
    // Valid executable names must report not_supported on WASI
    const auto direct = syscape::environment::find_executable("/bin/sh");
    expect(
        !direct && direct.error() == syscape::errc::not_supported,
        "find_executable with explicit path must report not_supported on WASI");

    const auto path_based = syscape::environment::find_executable("sh");
    expect(
        !path_based && path_based.error() == syscape::errc::not_supported,
        "find_executable with command name must report not_supported on WASI");
