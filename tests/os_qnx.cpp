...
void test_parsing_functions() {
    ...
    // Hostname parsing
    {
        const auto res = parse_host_name("qnxbox");
        expect(res.has_value() && *res == "qnxbox",
               "valid hostname 'qnxbox' must parse");
    }
    {
...
void test_runtime_queries() {
    expect(syscape::target_operating_system() == syscape::operating_system::qnx,
           "target_operating_system must report qnx");
    expect(syscape::target_execution_environment() ==
               syscape::execution_environment::rtos,
           "target_execution_environment must report rtos for QNX");

    const auto prod = syscape::os::product_name();
    expect(prod.has_value() && *prod == "QNX Neutrino RTOS",
           "product name must be 'QNX Neutrino RTOS'");
    ...
    expect(build.error() == syscape::errc::not_supported,
           "build identifier must report not_supported on QNX");

    const auto boot_id = syscape::os::boot_identifier();
    expect(boot_id.error() == syscape::errc::not_supported,
           "boot identifier must report not_supported on QNX");
}
