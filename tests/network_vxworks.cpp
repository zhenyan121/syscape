...
void test_dns_parsing() {
    ...
    const auto bad_ns = syscape::detail::network_backend::parse_dns_content(
        "nameserver 1.1.1.1 garbage\n");
    expect(!bad_ns && bad_ns.error() == syscape::errc::malformed_data,
           "nameserver with trailing garbage must fail with malformed_data");

    const auto empty_ns =
        syscape::detail::network_backend::parse_dns_content("nameserver\n");
    expect(!empty_ns && empty_ns.error() == syscape::errc::malformed_data,
           "empty nameserver directive must fail with malformed_data");

    const auto empty_search =
        syscape::detail::network_backend::parse_dns_content("search\n");
    expect(!empty_search &&
               empty_search.error() == syscape::errc::malformed_data,
           "empty search directive must fail with malformed_data");

    const auto bad_domain = syscape::detail::network_backend::parse_dns_content(
        "domain example.com garbage\n");
    expect(!bad_domain && bad_domain.error() == syscape::errc::malformed_data,
           "domain with trailing garbage must fail with malformed_data");

    const auto empty_domain =
        syscape::detail::network_backend::parse_dns_content("domain\n");
    expect(!empty_domain &&
               empty_domain.error() == syscape::errc::malformed_data,
           "empty domain directive must fail with malformed_data");
}

void test_shortened_netmask_copy() {
    const unsigned char compact_ipv4_mask[] = {255U, 255U, 0U, 0U};
    unsigned char copied[4] = {1U, 2U, 3U, 4U};
    // Netmask with recorded length 2 (only first 2 bytes present in source)
    syscape::detail::network_backend::copy_recorded_netmask_bytes(
        compact_ipv4_mask, 2U, 0U, copied, sizeof(copied));
    expect(copied[0] == 255U && copied[1] == 255U && copied[2] == 0U &&
               copied[3] == 0U,
           "truncated netmask must be zero-filled after recorded length");
}
...
void test_network_queries() {
    ...
    expect(stats.error() == syscape::errc::not_supported,
           "statistics query must report not_supported on VxWorks");
}
