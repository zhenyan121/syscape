...
void test_process_list_queries() {
    ...
    if (procs) {
        ...
        for (const auto& proc : *procs) {
            expect(!proc.uid.has_value(), "uid must remain nullopt on QNX");
            expect(!proc.gid.has_value(), "gid must remain nullopt on QNX");
            expect(!proc.user_name.has_value(),
                   "user_name must remain nullopt on QNX");
            expect(!proc.ppid.has_value(), "ppid must remain nullopt on QNX");
        }
    }
