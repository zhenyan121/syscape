...
int main() {
    ...
    {
        const auto sp = syscape::filesystem::space("ADFS::0.$");
        assert(!sp.has_value());
        ...
        assert(max_comp->length == 255U);

        const auto max_path = syscape::filesystem::max_path_length("ADFS::0.$");
