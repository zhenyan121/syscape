...
int main() {
    ...
    {
        const auto sp = syscape::filesystem::space("DH0:");
        assert(!sp.has_value());
        assert(sp.error() == syscape::errc::not_supported);

        const auto max_comp = syscape::filesystem::max_component_length("DH0:");
        assert(max_comp.has_value());
        assert(max_comp->length == 30U);

        const auto max_path = syscape::filesystem::max_path_length("DH0:");
        assert(max_path.has_value());
        assert(max_path->length == 255U);
    ...
    // 7. Delegating headers return not_supported
    {
