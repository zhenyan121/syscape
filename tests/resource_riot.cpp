...
void test_resource_queries() {
    ...
    expect(threads.has_value() && *threads == 4U,
           "thread count must match mock thread count on RIOT OS");

    riot_mock_set_thread_count(8);
