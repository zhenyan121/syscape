...
void test_parsing_functions() {
    ...
    // Hostname parsing
    {
        const auto res = parse_host_name("rtemsbox");
        expect(res.has_value() && *res == "rtemsbox",
               "valid hostname 'rtemsbox' must parse");
    }
    {
...
void test_runtime_queries() {
    expect(syscape::target_operating_system() ==
               syscape::operating_system::rtems,
           "target_operating_system must report rtems");
    expect(syscape::target_execution_environment() ==
               syscape::execution_environment::rtos,
           "target_execution_environment must report rtos for RTEMS");

    const auto prod = syscape::os::product_name();
    expect(prod.has_value() && *prod == "RTEMS",
           "product name must be 'RTEMS'");
    ...
    expect(build.error() == syscape::errc::not_supported,
           "build identifier must report not_supported on RTEMS");

    const auto boot_id = syscape::os::boot_identifier();
    expect(boot_id.error() == syscape::errc::not_supported,
           "boot identifier must report not_supported on RTEMS");
}
