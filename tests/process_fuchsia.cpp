...
void test_process_queries() {
    ...
    expect(pid && *pid != 0, "process id must be non-zero");
    ...
    expect(!threads && threads.error() == syscape::errc::not_supported,
           "thread count query must report not_supported on Fuchsia");
    ...
    expect(cmdline.error() == syscape::errc::not_supported,
           "command line must report not_supported on Fuchsia");
    ...
    expect(mem.error() == syscape::errc::not_supported,
           "memory usage must report not_supported on Fuchsia");
