...
void test_environment_queries() {
    ...
    expect((tmp && !tmp->empty() && tmp->front() == '/') ||
           ...
           "temp directory must be an absolute path or not_found");
...
void test_relative_temp_dir_rejected() {
    ::setenv("TMPDIR", ".", 1);
    const auto tmp = syscape::environment::temp_directory();
    expect(!tmp || tmp->front() == '/',
           "temp directory must never return a relative path even if TMPDIR is "
           "relative");
}
...
int main() {
    ...
    test_relative_temp_dir_rejected();
