...
void test_power_queries() {
    ...
    expect(bats.has_value() ||
               bats.error() == syscape::errc::permission_denied ||
               bats.error() == syscape::errc::not_supported,
           "batteries query must succeed, report permission_denied, or report "
           "not_supported");

    const auto ext = syscape::power::external_power_online();
    expect(ext.has_value() || ext.error() == syscape::errc::not_supported ||
               ext.error() == syscape::errc::permission_denied ||
               ext.error() == syscape::errc::not_found,
           "external power query must succeed, report not_supported, "
