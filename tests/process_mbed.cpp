...
void test_process_queries() {
    ...
    expect(prio.has_value() && *prio == static_cast<int>(osPriorityNormal),
           "priority must match mock thread priority on Mbed OS");

    mbed_mock_set_priority(osPriorityHigh);
    const auto prio2 = syscape::process::priority();
    expect(prio2.has_value() && *prio2 == static_cast<int>(osPriorityHigh),
           "priority must dynamically reflect updated priority");

    mbed_mock_set_priority(osPriorityError);
    const auto err_prio = syscape::process::priority();
    expect(!err_prio && err_prio.error() == syscape::errc::not_supported,
           "priority must report not_supported on osPriorityError");
    mbed_mock_set_priority(osPriorityNormal);

    const auto pid = syscape::process::process_id();
    expect(!pid && pid.error() == syscape::errc::not_supported,
           "process id must report not_supported on Mbed OS");

    const auto ppid = syscape::process::parent_process_id();
    expect(!ppid && ppid.error() == syscape::errc::not_supported,
           "parent process id must report not_supported on Mbed OS");

    const auto exe = syscape::process::executable_path();
    expect(!exe && exe.error() == syscape::errc::not_supported,
           "executable path must report not_supported on Mbed OS");

    const auto cmd = syscape::process::command_line();
    expect(!cmd && cmd.error() == syscape::errc::not_supported,
           "command line must report not_supported on Mbed OS");

    const auto cwd = syscape::process::working_directory();
    expect(!cwd && cwd.error() == syscape::errc::not_supported,
           "working directory must report not_supported on Mbed OS");

    const auto cpu = syscape::process::cpu_time();
    expect(!cpu && cpu.error() == syscape::errc::not_supported,
           "cpu time must report not_supported on Mbed OS");

    const auto start = syscape::process::start_time();
    expect(!start && start.error() == syscape::errc::not_supported,
           "start time must report not_supported on Mbed OS");
    ...
    expect(!aff && aff.error() == syscape::errc::not_supported,
           "cpu affinity must report not_supported on Mbed OS");
    ...
    expect(!lim && lim.error() == syscape::errc::not_supported,
           "resource limit must report not_supported on Mbed OS");
}
