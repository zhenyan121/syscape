...
void test_process_list_queries() {
    ...
    expect((matches.has_value() && matches->empty()) ||
               matches.error() == syscape::errc::not_supported ||
               matches.error() == syscape::errc::permission_denied,
    ...
    expect((empty_matches.has_value() && empty_matches->empty()) ||
               empty_matches.error() == syscape::errc::not_supported ||
               empty_matches.error() == syscape::errc::permission_denied,
           "find_processes_by_name for empty name must return empty list or "
           "error");
}
