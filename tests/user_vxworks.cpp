...
void test_user_queries() {
    ...
    expect(name.error() == syscape::errc::not_supported,
           "user name query must report not_supported on VxWorks");

    const auto home = syscape::user::home_directory();
    expect(home.error() == syscape::errc::not_supported,
           "user home directory query must report not_supported on VxWorks");

    const auto sh = syscape::user::shell();
    expect(sh.error() == syscape::errc::not_supported,
           "user shell query must report not_supported on VxWorks");
