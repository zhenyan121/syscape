...
void test_user_queries() {
    ...
    expect((login && !login->empty()) ||
           ...
           "login name query must succeed, not_found, or not_supported");
    ...
    expect(logged_in.has_value() ||
               logged_in.error() == syscape::errc::not_supported,
           "logged-in users query must succeed or report not_supported");
}
