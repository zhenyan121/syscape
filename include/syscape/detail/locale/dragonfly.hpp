#ifndef SYSCAPE_DETAIL_LOCALE_DRAGONFLY_HPP
#define SYSCAPE_DETAIL_LOCALE_DRAGONFLY_HPP
...
inline result<std::string> time_zone_identifier() {
    ...
    if (configured == nullptr) {
        ...
        const result<std::string> saved_db =
            read_small_file("/var/db/zoneinfo");
        if (saved_db) {
            return checked_zone_identifier(*saved_db);
        }
