#ifndef SYSCAPE_DETAIL_ENVIRONMENT_APPLE_MOBILE_HPP
#define SYSCAPE_DETAIL_ENVIRONMENT_APPLE_MOBILE_HPP
...
inline result<std::string> temp_directory() {
    ...
    for (std::size_t attempt = 0; attempt < 5U; ++attempt) {
        ...
        const std::size_t required =
            ::confstr(_CS_DARWIN_USER_TEMP_DIR, buffer.data(), buffer.size());
        if (required == 0U) {
            return fail(errno != 0
