#ifndef SYSCAPE_DETAIL_PRINTER_OPENBSD_HPP
#define SYSCAPE_DETAIL_PRINTER_OPENBSD_HPP
...
read_text_file(const char* path, std::size_t maximum_size = 1024U * 1024U) {
    ...
    const openbsd_file_descriptor owned_descriptor(descriptor);
    ...
    while (line_start < content.size()) {
        ...
        if (line.back() == '\\') {
            ...
            if (colon_pos != std::string::npos && colon_pos > 0) {
                ...
                if (!primary_name.empty() && is_valid_utf8(primary_name)) {
