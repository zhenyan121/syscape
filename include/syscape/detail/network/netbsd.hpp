#ifndef SYSCAPE_DETAIL_NETWORK_NETBSD_HPP
#define SYSCAPE_DETAIL_NETWORK_NETBSD_HPP
...
class netbsd_file_descriptor {
    public:
    explicit netbsd_file_descriptor(int value) noexcept : value_(value) {}
    netbsd_file_descriptor(const netbsd_file_descriptor&) = delete;
    netbsd_file_descriptor& operator=(const netbsd_file_descriptor&) = delete;
    ~netbsd_file_descriptor() {
        if (value_ >= 0) {
            ::close(value_);
        }
    }
...
inline result<network_common::dns_record> dns() {
    ...
    const netbsd_file_descriptor owned_fd(fd);
