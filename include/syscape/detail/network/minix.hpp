#ifndef SYSCAPE_DETAIL_NETWORK_MINIX_HPP
#define SYSCAPE_DETAIL_NETWORK_MINIX_HPP
    ...
    while (offset < content.size()) {
        ...
        const std::string_view line =
            (next_newline == std::string_view::npos)
                ...
                : content.substr(offset, next_newline - offset);
        offset = (next_newline == std::string_view::npos) ? content.size()
                                                          : next_newline + 1U;
        ...
        if (tokens[0] == "nameserver") {
            ...
            for (std::size_t index = 1U; index < tokens.size(); ++index) {
                search_list.emplace_back(tokens[index]);
            }
...
inline result<network_common::dns_record> dns() {
    auto content = read_file_to_string("/etc/resolv.conf");
    if (!content) {
