#ifndef SYSCAPE_DETAIL_NETWORK_HAIKU_HPP
#define SYSCAPE_DETAIL_NETWORK_HAIKU_HPP
...
inline result<std::string> read_file_content(const char* path) {
...
inline result<network_common::dns_record> dns() {
    auto content =
        read_file_content("/boot/system/settings/network/resolv.conf");
    if (!content) {
        content = read_file_content("/etc/resolv.conf");
    }
    if (!content) {
