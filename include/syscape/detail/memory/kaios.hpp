...
inline result<std::uint64_t> page_size_bytes() {
    ...
    return validate_positive_u64_bytes(SYSCAPE_KAIOS_PAGE_SIZE_BYTES);
#else
    errno = 0;
...
inline result<std::uint64_t> physical_memory_bytes() {
    ...
    return validate_positive_u64_bytes(SYSCAPE_KAIOS_TOTAL_RAM_BYTES);
#elif defined(SYSCAPE_KAIOS_PHYSICAL_MEMORY_BYTES)
    return validate_positive_u64_bytes(SYSCAPE_KAIOS_PHYSICAL_MEMORY_BYTES);
#else
...
inline result<std::uint64_t> available_memory_bytes() {
    ...
    return validate_non_negative_u64_bytes(SYSCAPE_KAIOS_FREE_RAM_BYTES);
#elif defined(SYSCAPE_KAIOS_AVAILABLE_MEMORY_BYTES)
    return validate_non_negative_u64_bytes(
        SYSCAPE_KAIOS_AVAILABLE_MEMORY_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
