#ifndef SYSCAPE_DETAIL_MEMORY_KAIOS_HPP
#define SYSCAPE_DETAIL_MEMORY_KAIOS_HPP
...
#if defined(SYSCAPE_KAIOS_PAGE_SIZE_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_KAIOS_PAGE_SIZE_BYTES);
...
#if defined(SYSCAPE_KAIOS_TOTAL_RAM_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_KAIOS_TOTAL_RAM_BYTES);
#elif defined(SYSCAPE_KAIOS_PHYSICAL_MEMORY_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_KAIOS_PHYSICAL_MEMORY_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_KAIOS_FREE_RAM_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_KAIOS_FREE_RAM_BYTES);
#elif defined(SYSCAPE_KAIOS_AVAILABLE_MEMORY_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_KAIOS_AVAILABLE_MEMORY_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
