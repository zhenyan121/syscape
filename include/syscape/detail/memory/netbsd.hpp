#ifndef SYSCAPE_DETAIL_MEMORY_NETBSD_HPP
#define SYSCAPE_DETAIL_MEMORY_NETBSD_HPP
...
inline result<struct uvmexp_sysctl> get_uvmexp2() {
...
inline result<std::uint64_t> physical_memory_bytes() {
    ...
    int phys32 = 0;
    std::size_t size32 = sizeof(phys32);
    if (::sysctl(mib32, 2U, &phys32, &size32, nullptr, 0U) == 0) {
        if (size32 == sizeof(phys32) && phys32 > 0) {
...
#if defined(VM_UVMEXP2)
    const auto uvm2 = get_uvmexp2();
    if (uvm2) {
        if (uvm2->pagesize <= 0 || uvm2->npages <= 0) {
            return fail(errc::malformed_data);
        }
        constexpr std::uint64_t max_u64 =
            (std::numeric_limits<std::uint64_t>::max)();
        const std::uint64_t npages = static_cast<std::uint64_t>(uvm2->npages);
        const std::uint64_t page_size =
            static_cast<std::uint64_t>(uvm2->pagesize);
        if (npages > max_u64 / page_size) {
    ...
    constexpr std::uint64_t max_u64 =
        (std::numeric_limits<std::uint64_t>::max)();
    const std::uint64_t npages = static_cast<std::uint64_t>(uvm->npages);
...
inline result<std::uint64_t> available_memory_bytes() {
    ...
    const auto uvm2 = get_uvmexp2();
    if (uvm2) {
        if (uvm2->pagesize <= 0 || uvm2->free < 0 || uvm2->inactive < 0) {
            return fail(errc::malformed_data);
        }
        const std::uint64_t free_pages = static_cast<std::uint64_t>(uvm2->free);
        const std::uint64_t inactive_pages =
            static_cast<std::uint64_t>(uvm2->inactive);
        const std::uint64_t page_size =
            static_cast<std::uint64_t>(uvm2->pagesize);
...
inline result<memory_common::swap_usage> swap_status() {
    ...
    const auto uvm2 = get_uvmexp2();
    if (uvm2) {
        if (uvm2->pagesize <= 0 || uvm2->swpages < 0 || uvm2->swpginuse < 0 ||
            uvm2->swpginuse > uvm2->swpages) {
            return fail(errc::malformed_data);
        }
