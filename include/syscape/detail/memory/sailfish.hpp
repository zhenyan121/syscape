...
inline result<std::uint64_t> page_size_bytes() {
    ...
    return static_cast<std::uint64_t>(SYSCAPE_SAILFISH_PAGE_SIZE_BYTES);
...
inline result<std::uint64_t> physical_memory_bytes() {
    ...
    return static_cast<std::uint64_t>(SYSCAPE_SAILFISH_TOTAL_RAM_BYTES);
#elif defined(SYSCAPE_SAILFISH_PHYSICAL_MEMORY_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_SAILFISH_PHYSICAL_MEMORY_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint64_t> available_memory_bytes() {
    ...
    return static_cast<std::uint64_t>(SYSCAPE_SAILFISH_FREE_RAM_BYTES);
#elif defined(SYSCAPE_SAILFISH_AVAILABLE_MEMORY_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_SAILFISH_AVAILABLE_MEMORY_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
