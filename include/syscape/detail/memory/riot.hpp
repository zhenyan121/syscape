...
#if defined(RIOT_TOTAL_HEAP_SIZE) && (RIOT_TOTAL_HEAP_SIZE > 0)
    return static_cast<std::uint64_t>(RIOT_TOTAL_HEAP_SIZE);
#elif defined(RIOT_HEAP_SIZE) && (RIOT_HEAP_SIZE > 0)
    return static_cast<std::uint64_t>(RIOT_HEAP_SIZE);
#elif defined(CONFIG_HEAP_SIZE) && (CONFIG_HEAP_SIZE > 0)
    return static_cast<std::uint64_t>(CONFIG_HEAP_SIZE);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(RIOT_FREE_HEAP_SIZE)
    return static_cast<std::uint64_t>(RIOT_FREE_HEAP_SIZE);
#elif defined(CONFIG_FREE_HEAP_SIZE)
    return static_cast<std::uint64_t>(CONFIG_FREE_HEAP_SIZE);
#else
    return fail(errc::not_supported);
#endif
}
