#ifndef SYSCAPE_DETAIL_MEMORY_AMIGAOS_HPP
#define SYSCAPE_DETAIL_MEMORY_AMIGAOS_HPP
...
#if defined(SYSCAPE_AMIGAOS_TOTAL_RAM_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_AMIGAOS_TOTAL_RAM_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_AMIGAOS_FREE_RAM_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_AMIGAOS_FREE_RAM_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
