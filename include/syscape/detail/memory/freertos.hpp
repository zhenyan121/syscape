#ifndef SYSCAPE_DETAIL_MEMORY_FREERTOS_HPP
#define SYSCAPE_DETAIL_MEMORY_FREERTOS_HPP
...
// Returns the total FreeRTOS heap capacity configured by configTOTAL_HEAP_SIZE.
// Note: On embedded MCU targets this represents the FreeRTOS heap rather than
// the total hardware physical RAM/SRAM size.
inline result<std::uint64_t> physical_memory_bytes() {
#if defined(configTOTAL_HEAP_SIZE)
    return static_cast<std::uint64_t>(configTOTAL_HEAP_SIZE);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint64_t> available_memory_bytes() {
    ...
    return static_cast<std::uint64_t>(::xPortGetFreeHeapSize());
#else
    return fail(errc::not_supported);
#endif
}
