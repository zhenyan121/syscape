#ifndef SYSCAPE_DETAIL_MEMORY_RISCOS_HPP
#define SYSCAPE_DETAIL_MEMORY_RISCOS_HPP
...
#if defined(SYSCAPE_RISCOS_TOTAL_RAM_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_RISCOS_TOTAL_RAM_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_RISCOS_FREE_RAM_BYTES)
    return static_cast<std::uint64_t>(SYSCAPE_RISCOS_FREE_RAM_BYTES);
#else
    return fail(errc::not_supported);
#endif
}
