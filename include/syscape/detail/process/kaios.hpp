#ifndef SYSCAPE_DETAIL_PROCESS_KAIOS_HPP
#define SYSCAPE_DETAIL_PROCESS_KAIOS_HPP
...
#if defined(SYSCAPE_KAIOS_PID)
    return static_cast<std::uint32_t>(SYSCAPE_KAIOS_PID);
#else
...
#if defined(SYSCAPE_KAIOS_PPID)
    return static_cast<std::uint32_t>(SYSCAPE_KAIOS_PPID);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_KAIOS_THREAD_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_KAIOS_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
