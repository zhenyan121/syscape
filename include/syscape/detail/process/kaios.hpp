...
inline result<std::uint32_t> process_id() {
    ...
    return validate_positive_u32_count(SYSCAPE_KAIOS_PID);
#else
...
inline result<std::uint32_t> parent_process_id() {
    ...
    return validate_u32_id(SYSCAPE_KAIOS_PPID);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint32_t> thread_count() {
    ...
    return validate_positive_u32_count(SYSCAPE_KAIOS_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
