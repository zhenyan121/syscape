#ifndef SYSCAPE_DETAIL_PROCESS_MBED_HPP
#define SYSCAPE_DETAIL_PROCESS_MBED_HPP
...
inline result<int> priority() {
    ...
    const auto id = osThreadGetId();
    if (id != nullptr) {
        const auto prio = osThreadGetPriority(id);
        if (static_cast<int>(prio) >= 0) {
