#ifndef SYSCAPE_DETAIL_PROCESS_OPENVMS_HPP
#define SYSCAPE_DETAIL_PROCESS_OPENVMS_HPP
...
#if defined(SYSCAPE_OPENVMS_PID)
    return static_cast<std::uint32_t>(SYSCAPE_OPENVMS_PID);
#else
...
#if defined(SYSCAPE_OPENVMS_PPID)
    return static_cast<std::uint32_t>(SYSCAPE_OPENVMS_PPID);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_OPENVMS_THREAD_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_OPENVMS_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_OPENVMS_TASK_PRIORITY)
    return static_cast<int>(SYSCAPE_OPENVMS_TASK_PRIORITY);
#else
    return fail(errc::not_supported);
#endif
}
