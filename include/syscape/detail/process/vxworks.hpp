#ifndef SYSCAPE_DETAIL_PROCESS_VXWORKS_HPP
#define SYSCAPE_DETAIL_PROCESS_VXWORKS_HPP
...
inline result<int> validate_priority(int prio) noexcept {
    if (prio < 0 || prio > 255) {
        return fail(errc::malformed_data);
    }
    return prio;
...
inline result<int> priority() {
    ...
    return validate_priority(param.sched_priority);
}
