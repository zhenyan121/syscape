#ifndef SYSCAPE_DETAIL_PROCESS_IBMI_HPP
#define SYSCAPE_DETAIL_PROCESS_IBMI_HPP
...
#if defined(SYSCAPE_IBMI_PID)
    return static_cast<std::uint32_t>(SYSCAPE_IBMI_PID);
#else
...
#if defined(SYSCAPE_IBMI_PPID)
    return static_cast<std::uint32_t>(SYSCAPE_IBMI_PPID);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_IBMI_THREAD_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_IBMI_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_IBMI_TASK_PRIORITY)
    return static_cast<int>(SYSCAPE_IBMI_TASK_PRIORITY);
#else
    return fail(errc::not_supported);
#endif
}
