#ifndef SYSCAPE_DETAIL_PROCESS_ZOS_HPP
#define SYSCAPE_DETAIL_PROCESS_ZOS_HPP
...
#if defined(SYSCAPE_ZOS_PID)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_PID);
#else
...
#if defined(SYSCAPE_ZOS_PPID)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_PPID);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_ZOS_THREAD_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_ZOS_TASK_PRIORITY)
    return static_cast<int>(SYSCAPE_ZOS_TASK_PRIORITY);
#else
    return fail(errc::not_supported);
#endif
}
