#ifndef SYSCAPE_DETAIL_PROCESS_OPENHARMONY_HPP
#define SYSCAPE_DETAIL_PROCESS_OPENHARMONY_HPP
...
inline result<std::vector<std::string>> command_line() {
    const auto content = openharmony::read_text_file("/proc/self/cmdline");
    if (!content) {
...
inline result<std::chrono::system_clock::time_point> start_time() {
    const auto content = openharmony::read_text_file("/proc/self/stat");
    if (!content) {
        return fail(content.error());
    }
...
inline result<process_common::memory_usage_snapshot> memory_usage() {
    const auto content = openharmony::read_text_file("/proc/self/statm");
    if (!content) {
        return fail(content.error());
    }

    std::string_view text = *content;
    openharmony::strip_trailing_newlines(text);
