#ifndef SYSCAPE_DETAIL_PROCESS_FREEBSD_HPP
#define SYSCAPE_DETAIL_PROCESS_FREEBSD_HPP
...
inline result<std::vector<std::string>> command_line() {
    ...
    constexpr std::size_t maximum_size = 16U * 1024U * 1024U;
...
inline result<std::chrono::system_clock::time_point> start_time() {
    ...
    return timeval_to_time_point(
        static_cast<std::int64_t>(kp->ki_start.tv_sec),
        static_cast<std::int64_t>(kp->ki_start.tv_usec));
}
...
inline result<process_common::memory_usage_snapshot> memory_usage() {
    ...
    const std::uint64_t resident_pages =
        static_cast<std::uint64_t>(kp->ki_rssize);
    ...
    mem.virtual_bytes = static_cast<std::uint64_t>(kp->ki_size);
    return mem;
}
...
inline result<std::uint32_t> thread_count() {
    ...
    return static_cast<std::uint32_t>(kp->ki_numthreads);
}
