#ifndef SYSCAPE_DETAIL_PROCESS_TKERNEL_HPP
#define SYSCAPE_DETAIL_PROCESS_TKERNEL_HPP
...
inline result<int> priority() {
    ...
    T_RTSK rtsk {};
#if defined(TSK_SELF)
    const ID tskid = TSK_SELF;
#else
    const ID tskid = 0;
#endif
    if (::tk_ref_tsk(tskid, &rtsk) != 0) {
        return fail(errc::not_supported);
    }
    return static_cast<int>(rtsk.tskpri);
#else
    return fail(errc::not_supported);
#endif
}
