...
inline result<std::uint32_t> process_id() {
    ...
    return static_cast<std::uint32_t>(SYSCAPE_SAILFISH_PID);
#else
...
inline result<std::uint32_t> parent_process_id() {
    ...
    return static_cast<std::uint32_t>(SYSCAPE_SAILFISH_PPID);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint32_t> thread_count() {
    ...
    return static_cast<std::uint32_t>(SYSCAPE_SAILFISH_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<int> priority() {
    ...
    return static_cast<int>(SYSCAPE_SAILFISH_TASK_PRIORITY);
#elif defined(SYSCAPE_SAILFISH_PRIORITY)
    return static_cast<int>(SYSCAPE_SAILFISH_PRIORITY);
#else
    return fail(errc::not_supported);
#endif
}
