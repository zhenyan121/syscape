#ifndef SYSCAPE_DETAIL_PROCESS_UCOS_HPP
#define SYSCAPE_DETAIL_PROCESS_UCOS_HPP
...
inline result<int> priority() {
    ...
    if (::OSTCBCurPtr == nullptr) {
        return fail(errc::not_supported);
    }
    return static_cast<int>(::OSTCBCurPtr->Prio);
#else
    if (::OSTCBCur == nullptr) {
        return fail(errc::not_supported);
    }
    return static_cast<int>(::OSTCBCur->OSTCBPrio);
#endif
#else
    return fail(errc::not_supported);
#endif
