#ifndef SYSCAPE_DETAIL_PROCESS_RTEMS_HPP
#define SYSCAPE_DETAIL_PROCESS_RTEMS_HPP
...
inline result<process_common::cpu_time_usage> cpu_time() {
    // RTEMS times() reports current-thread CPU time in tms_utime and system
    // uptime in tms_stime. Neither value satisfies the process-wide user and
    // system CPU-time semantics of the portable query.
    return fail(errc::not_supported);
}
