#ifndef SYSCAPE_DETAIL_RESOURCE_MYNEWT_HPP
#define SYSCAPE_DETAIL_RESOURCE_MYNEWT_HPP
...
inline result<std::uint64_t> thread_count() {
    ...
    return static_cast<std::uint64_t>(os_task_count());
#elif defined(MYNEWT_TASK_COUNT) && (MYNEWT_TASK_COUNT > 0)
    return static_cast<std::uint64_t>(MYNEWT_TASK_COUNT);
#elif defined(MYNEWT_MAX_TASKS) && (MYNEWT_MAX_TASKS > 0)
    return static_cast<std::uint64_t>(MYNEWT_MAX_TASKS);
#elif defined(OS_TASK_COUNT) && (OS_TASK_COUNT > 0)
    return static_cast<std::uint64_t>(OS_TASK_COUNT);
#else
    return fail(errc::not_supported);
#endif
