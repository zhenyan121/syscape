...
inline result<std::uint64_t> process_count() {
    ...
    return static_cast<std::uint64_t>(SYSCAPE_TIZEN_PROCESS_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint64_t> thread_count() {
    ...
    return static_cast<std::uint64_t>(SYSCAPE_TIZEN_SYSTEM_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint64_t> file_descriptor_limit() {
    ...
    return static_cast<std::uint64_t>(SYSCAPE_TIZEN_MAX_FILES);
#else
    return fail(errc::not_supported);
#endif
}
