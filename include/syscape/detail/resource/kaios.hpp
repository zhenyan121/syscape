#ifndef SYSCAPE_DETAIL_RESOURCE_KAIOS_HPP
#define SYSCAPE_DETAIL_RESOURCE_KAIOS_HPP
...
#if defined(SYSCAPE_KAIOS_PROCESS_COUNT)
    return validate_positive_u64_count(SYSCAPE_KAIOS_PROCESS_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_KAIOS_SYSTEM_THREAD_COUNT)
    return validate_positive_u64_count(SYSCAPE_KAIOS_SYSTEM_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_KAIOS_MAX_FILES)
    return validate_positive_u64_count(SYSCAPE_KAIOS_MAX_FILES);
#else
    return fail(errc::not_supported);
#endif
}
