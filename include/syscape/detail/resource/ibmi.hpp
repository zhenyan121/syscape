#ifndef SYSCAPE_DETAIL_RESOURCE_IBMI_HPP
#define SYSCAPE_DETAIL_RESOURCE_IBMI_HPP
...
#if defined(SYSCAPE_IBMI_PROCESS_COUNT)
    return static_cast<std::uint64_t>(SYSCAPE_IBMI_PROCESS_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_IBMI_SYSTEM_THREAD_COUNT)
    return static_cast<std::uint64_t>(SYSCAPE_IBMI_SYSTEM_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_IBMI_MAX_FILES)
    return static_cast<std::uint64_t>(SYSCAPE_IBMI_MAX_FILES);
#else
    return fail(errc::not_supported);
#endif
}
