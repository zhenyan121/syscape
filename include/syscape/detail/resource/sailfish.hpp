#ifndef SYSCAPE_DETAIL_RESOURCE_SAILFISH_HPP
#define SYSCAPE_DETAIL_RESOURCE_SAILFISH_HPP
...
#if defined(SYSCAPE_SAILFISH_PROCESS_COUNT)
    return validate_positive_u64_count(SYSCAPE_SAILFISH_PROCESS_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_SAILFISH_SYSTEM_THREAD_COUNT)
    return validate_positive_u64_count(SYSCAPE_SAILFISH_SYSTEM_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_SAILFISH_MAX_FILES)
    return validate_positive_u64_count(SYSCAPE_SAILFISH_MAX_FILES);
#else
    return fail(errc::not_supported);
#endif
}
