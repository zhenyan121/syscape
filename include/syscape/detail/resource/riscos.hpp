#ifndef SYSCAPE_DETAIL_RESOURCE_RISCOS_HPP
#define SYSCAPE_DETAIL_RESOURCE_RISCOS_HPP
...
#if defined(SYSCAPE_RISCOS_PROCESS_COUNT)
    return static_cast<std::uint64_t>(SYSCAPE_RISCOS_PROCESS_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_RISCOS_SYSTEM_THREAD_COUNT)
    return static_cast<std::uint64_t>(SYSCAPE_RISCOS_SYSTEM_THREAD_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_RISCOS_MAX_FILES)
    return static_cast<std::uint64_t>(SYSCAPE_RISCOS_MAX_FILES);
#else
    return fail(errc::not_supported);
#endif
}
