#ifndef SYSCAPE_DETAIL_RESOURCE_ANDROID_HPP
#define SYSCAPE_DETAIL_RESOURCE_ANDROID_HPP
...
inline result<resource_common::load_samples> load_average() {
    const result<std::string> content =
        android::read_text_file("/proc/loadavg");
    if (!content) {
...
inline result<std::uint64_t> process_count() {
    android::directory_handle dir("/proc");
    if (!dir.valid()) {
...
inline result<std::uint64_t> file_descriptor_limit() {
    const result<std::string> content =
        android::read_text_file("/proc/sys/fs/file-max");
    if (content) {
    ...
    const result<std::string> nr_content =
        android::read_text_file("/proc/sys/fs/file-nr");
    if (nr_content) {
