#ifndef SYSCAPE_DETAIL_SENSOR_ANDROID_HPP
#define SYSCAPE_DETAIL_SENSOR_ANDROID_HPP
...
inline result<std::vector<::syscape::sensor::thermal_zone>> thermal_zones() {
    android::directory_handle dir("/sys/class/thermal");
    if (!dir.valid()) {
        if (dir.error() == ENOENT) {
    ...
    for (;;) {
        ...
        const auto temp_content = android::read_text_file(temp_path.c_str());
        if (!temp_content) {
            if (temp_content.error() == errc::not_found) {
                // Do not fabricate a 0.0 C temperature for a zone without temp
                // reading
                continue;
            }
        ...
        android::strip_trailing_newlines(temp_sv);
        while (!temp_sv.empty() &&
