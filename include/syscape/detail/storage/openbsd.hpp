#ifndef SYSCAPE_DETAIL_STORAGE_OPENBSD_HPP
#define SYSCAPE_DETAIL_STORAGE_OPENBSD_HPP
...
classify_bus_name(std::string_view name) noexcept {
    ...
    if (name.rfind("vio", 0) == 0 || name.rfind("vnd", 0) == 0) {
...
inline result<std::vector<storage_common::drive_record>> drives() {
    ...
    while (start < disks_str->size()) {
        std::size_t comma = disks_str->find(',', start);
        std::string token = (comma == std::string::npos)
                                ? disks_str->substr(start)
                                : disks_str->substr(start, comma - start);
        start = (comma == std::string::npos) ? disks_str->size() : comma + 1U;
