#ifndef SYSCAPE_DETAIL_STORAGE_OPENHARMONY_HPP
#define SYSCAPE_DETAIL_STORAGE_OPENHARMONY_HPP
...
inline result<std::vector<storage_common::drive_record>> drives() {
    openharmony::directory_handle dir("/sys/block");
    if (!dir.valid()) {
    ...
    for (;;) {
        ...
        const auto size_content =
            openharmony::read_text_file(size_path.c_str());
        if (!size_content) {
        ...
        const auto rot_content = openharmony::read_text_file(rot_path.c_str());
        if (rot_content) {
            std::string_view rot_sv = *rot_content;
            openharmony::strip_trailing_newlines(rot_sv);
            if (rot_sv == "1") {
