...
inline result<std::uint32_t> online_logical_processor_count() {
    ...
    return static_cast<std::uint32_t>(SYSCAPE_TIZEN_CPU_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint32_t> online_physical_core_count() {
    ...
    return static_cast<std::uint32_t>(SYSCAPE_TIZEN_PHYSICAL_CPU_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
inline result<std::uint32_t> online_processor_package_count() {
    ...
    return static_cast<std::uint32_t>(SYSCAPE_TIZEN_PACKAGE_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
