#ifndef SYSCAPE_DETAIL_CPU_ZOS_HPP
#define SYSCAPE_DETAIL_CPU_ZOS_HPP
...
#if defined(SYSCAPE_ZOS_CPU_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_CPU_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
...
#if defined(SYSCAPE_ZOS_PHYSICAL_CPU_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_PHYSICAL_CPU_COUNT);
#elif defined(SYSCAPE_ZOS_CPU_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_CPU_COUNT);
#else
    return fail(errc::not_supported);
#endif
...
#if defined(SYSCAPE_ZOS_PACKAGE_COUNT)
    return static_cast<std::uint32_t>(SYSCAPE_ZOS_PACKAGE_COUNT);
#else
    return fail(errc::not_supported);
#endif
}
