#ifndef SYSCAPE_DETAIL_CPU_OPENBSD_HPP
#define SYSCAPE_DETAIL_CPU_OPENBSD_HPP
...
inline result<cpu_common::usage_information> cumulative_processor_usage() {
    ...
    const std::uint64_t user = static_cast<std::uint64_t>(cp_time[CP_USER]);
    const std::uint64_t nice = static_cast<std::uint64_t>(cp_time[CP_NICE]);
    const std::uint64_t sys = static_cast<std::uint64_t>(cp_time[CP_SYS]);
#ifdef CP_SPIN
    const std::uint64_t spin = static_cast<std::uint64_t>(cp_time[CP_SPIN]);
#else
    const std::uint64_t spin = 0U;
#endif
    const std::uint64_t intr = static_cast<std::uint64_t>(cp_time[CP_INTR]);
    const std::uint64_t idle = static_cast<std::uint64_t>(cp_time[CP_IDLE]);

    constexpr std::uint64_t max_u64 =
        (std::numeric_limits<std::uint64_t>::max)();
    if (user > max_u64 - nice || sys > max_u64 - spin ||
        sys + spin > max_u64 - intr) {
        return fail(errc::value_too_large);
    }

    cpu_common::usage_information usage;
    usage.user_ticks = user + nice;
    usage.system_ticks = sys + spin + intr;
