#ifndef SYSCAPE_DETAIL_PROCESS_LIST_ANDROID_HPP
#define SYSCAPE_DETAIL_PROCESS_LIST_ANDROID_HPP
...
make_entry_from_pid(std::uint32_t pid) {
    ...
    const auto stat_content =
        android::read_text_file((pid_dir + "/stat").c_str());
    if (!stat_content) {
    ...
    const auto cmdline =
        android::read_text_file((pid_dir + "/cmdline").c_str());
    if (cmdline) {
        if (!cmdline->empty()) {
