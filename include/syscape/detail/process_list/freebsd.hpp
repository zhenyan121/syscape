#ifndef SYSCAPE_DETAIL_PROCESS_LIST_FREEBSD_HPP
#define SYSCAPE_DETAIL_PROCESS_LIST_FREEBSD_HPP
...
inline process_list::process_state freebsd_process_state(char state) noexcept {
    switch (state) {
    case SRUN:
...
make_entry_from_kinfo(const struct kinfo_proc& kp, long page_size) {
    ...
    entry.pid = static_cast<std::uint32_t>(kp.ki_pid);
    entry.ppid = static_cast<std::uint32_t>(kp.ki_ppid);
    entry.uid = static_cast<std::uint32_t>(kp.ki_ruid);
    entry.gid = static_cast<std::uint32_t>(kp.ki_rgid);
    entry.priority = static_cast<int>(kp.ki_nice);

    if (kp.ki_login[0] != '\0') {
        std::string login(kp.ki_login);
        if (is_valid_utf8(login)) {
            entry.user_name = std::move(login);
        }
    }
    if (!entry.user_name && entry.uid) {
        entry.user_name = lookup_username_by_uid(*entry.uid);
    }

    std::string comm(kp.ki_comm);
    if (is_valid_utf8(comm)) {
        entry.name = std::move(comm);
    }

    entry.state = freebsd_process_state(kp.ki_stat);

    if (kp.ki_start.tv_sec > 0) {
        const auto secs = std::chrono::seconds(kp.ki_start.tv_sec);
        const auto usecs = std::chrono::microseconds(kp.ki_start.tv_usec);
        entry.start_time = std::chrono::system_clock::time_point(
            std::chrono::duration_cast<std::chrono::system_clock::duration>(
                secs + usecs));
    }

    const auto utime = std::chrono::seconds(kp.ki_rusage.ru_utime.tv_sec) +
                       std::chrono::microseconds(kp.ki_rusage.ru_utime.tv_usec);
    const auto stime = std::chrono::seconds(kp.ki_rusage.ru_stime.tv_sec) +
                       std::chrono::microseconds(kp.ki_rusage.ru_stime.tv_usec);
    entry.user_cpu_time =
    ...
    if (page_size > 0 && kp.ki_rssize > 0) {
        entry.resident_memory_bytes = static_cast<std::uint64_t>(kp.ki_rssize) *
                                      static_cast<std::uint64_t>(page_size);
    }
    if (kp.ki_size > 0) {
        entry.virtual_memory_bytes = static_cast<std::uint64_t>(kp.ki_size);
    }
    if (kp.ki_numthreads > 0) {
        entry.thread_count = static_cast<std::uint32_t>(kp.ki_numthreads);
    }

    entry.executable_path = get_process_exe(kp.ki_pid);
    entry.command_line = get_process_args(kp.ki_pid);
...
inline result<std::vector<struct kinfo_proc>> get_all_kinfo_procs() {
    ...
    std::size_t size = 0U;
    if (::sysctl(mib, 3, nullptr, &size, nullptr, 0U) != 0) {
    ...
    if (::sysctl(mib, 3, buffer.data(), &size, nullptr, 0U) != 0) {
        if (errno == EACCES || errno == EPERM) {
            return fail(errc::permission_denied);
        }
        return fail(std::error_code(errno, std::generic_category()));
