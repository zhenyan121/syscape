#ifndef SYSCAPE_DETAIL_SECURITY_ANDROID_HPP
#define SYSCAPE_DETAIL_SECURITY_ANDROID_HPP
...
inline result<::syscape::security::secure_boot_state> secure_boot() {
    const result<std::string> state =
        android::get_property("ro.boot.verifiedbootstate");
    if (state) {
    ...
    const result<std::string> vbmeta =
        android::get_property("ro.boot.vbmeta.device_state");
    if (vbmeta) {
        if (*vbmeta == "locked") {
            return ::syscape::security::secure_boot_state::enabled;
        }
...
inline result<::syscape::security::aslr_mode> aslr() {
    const auto content =
        android::read_text_file("/proc/sys/kernel/randomize_va_space");
    if (!content) {
