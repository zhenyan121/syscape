#ifndef SYSCAPE_DETAIL_OS_QNX_HPP
#define SYSCAPE_DETAIL_OS_QNX_HPP
...
inline result<std::string> product_name() {
    return std::string("QNX Neutrino RTOS");
}
...
inline result<std::string> host_name() {
    struct utsname uts;
    if (::uname(&uts) == 0 && uts.nodename[0] != '\0') {
        auto parsed = parse_host_name(uts.nodename);
        if (parsed) {
            return parsed;
        }
    }
