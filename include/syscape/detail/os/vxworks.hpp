#ifndef SYSCAPE_DETAIL_OS_VXWORKS_HPP
#define SYSCAPE_DETAIL_OS_VXWORKS_HPP
...
inline result<std::string> host_name() {
    ...
    while (buffer.size() <= maximum_size) {
        ...
        last_error = errno;
        if (last_error == ENAMETOOLONG || last_error == EINVAL) {
            buffer.resize(buffer.size() * 2U);
            continue;
        }
        break;
    ...
    if (last_error != 0 && last_error != ENOSYS && last_error != ENOTSUP) {
        return fail(std::error_code(last_error, std::generic_category()));
    }
    return fail(errc::not_found);
}
