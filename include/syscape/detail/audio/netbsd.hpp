#ifndef SYSCAPE_DETAIL_AUDIO_NETBSD_HPP
#define SYSCAPE_DETAIL_AUDIO_NETBSD_HPP
...
inline result<std::vector<::syscape::audio::audio_device>> devices() {
    ...
    for (int i = 0;; ++i) {
        ...
        std::string dname(adev.name, ::strnlen(adev.name, sizeof(adev.name)));
        ...
        if (adev.version[0] != '\0') {
            std::string dver(adev.version,
                             ::strnlen(adev.version, sizeof(adev.version)));
