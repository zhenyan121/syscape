#ifndef SYSCAPE_DETAIL_FILESYSTEM_APPLE_MOBILE_HPP
#define SYSCAPE_DETAIL_FILESYSTEM_APPLE_MOBILE_HPP
...
inline result<std::vector<filesystem_common::mount_record>> mounts() {
    ...
    if (static_cast<std::size_t>(initial_count) >
    ...
    for (;;) {
        ...
        if (::statfs(path.c_str(), &status) == 0) {
            static_assert(
                ...
                "The Apple backend requires 32-bit filesystem identifier "
                "words");
