...
max_component_length(const std::string& path) {
    (void)path;
#if defined(SYSCAPE_SAILFISH_NAME_MAX)
    const auto len = validate_positive_u64_len(SYSCAPE_SAILFISH_NAME_MAX);
    if (!len) {
...
max_path_length(const std::string& path) {
    ...
    const auto len = validate_positive_u64_len(SYSCAPE_SAILFISH_PATH_MAX);
    if (!len) {
        return fail(len.error());
    }
    filesystem_common::path_length_snapshot snap;
