...
/// Renders the kernel's recorded filesystem identifier pair through the
/// documented statfs interface with interrupted-call retries.
///
/// statfs reports f_fsid as two recorded words. Copying each word bit
/// by bit avoids any integer-sign conversion, and rendering indexes the
...
inline result<std::string> statfs_volume_id(const std::string& path) {
    for (;;) {
        ...
        if (::statfs(path.c_str(), &status) == 0) {
            static_assert(
                sizeof(status.f_fsid.__val[0]) == sizeof(std::uint32_t),
                "The Linux backend requires 32-bit statfs identifier words");
            std::uint32_t first = 0U;
            std::uint32_t second = 0U;
            ::std::memcpy(&first, &status.f_fsid.__val[0], sizeof(first));
            ::std::memcpy(&second, &status.f_fsid.__val[1],
                          sizeof(second));
