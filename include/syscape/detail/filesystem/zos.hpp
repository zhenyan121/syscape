#ifndef SYSCAPE_DETAIL_FILESYSTEM_ZOS_HPP
#define SYSCAPE_DETAIL_FILESYSTEM_ZOS_HPP
...
max_component_length(const std::string& /*path*/) {
    ...
    snap.length = 255U; // Standard zFS / HFS NAME_MAX
...
max_path_length(const std::string& /*path*/) {
    filesystem_common::path_length_snapshot snap;
    snap.length = 1024U; // Standard z/OS USS PATH_MAX
    snap.indeterminate = false;
    return snap;
}
