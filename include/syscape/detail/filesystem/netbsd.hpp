#ifndef SYSCAPE_DETAIL_FILESYSTEM_NETBSD_HPP
#define SYSCAPE_DETAIL_FILESYSTEM_NETBSD_HPP
...
convert_statvfs_entry(const struct ::statvfs& entry) {
    ...
    if (!is_valid_utf8(record.source) || !is_valid_utf8(record.mount_point) ||
        !is_valid_utf8(record.file_system_type)) {
...
inline result<std::vector<filesystem_common::mount_record>> mounts() {
    ...
    const int initial_count = ::getvfsstat(nullptr, 0, ST_NOWAIT);
    if (initial_count < 0) {
        return fail(std::error_code(errno, std::generic_category()));
    }
    std::size_t expected = static_cast<std::size_t>(initial_count) + 1U;
    const std::size_t maximum_entries =
        (std::numeric_limits<std::size_t>::max)() / sizeof(struct ::statvfs);
    if (expected > maximum_entries) {
        return fail(errc::value_too_large);
    }

    for (int attempt = 0; attempt < maximum_attempts; ++attempt) {
        std::vector<struct ::statvfs> entries(expected);
        const std::size_t requested_bytes =
            entries.size() * sizeof(struct ::statvfs);
        const int filled = ::getvfsstat(
            entries.data(), static_cast<size_t>(requested_bytes), ST_NOWAIT);
        if (filled < 0) {
            return fail(std::error_code(errno, std::generic_category()));
        }
        ...
        for (int index = 0; index < filled; ++index) {
            result<filesystem_common::mount_record> record =
                convert_statvfs_entry(entries[static_cast<std::size_t>(index)]);
            if (!record) {
...
inline result<std::string> statvfs_volume_id(const std::string& path) {
    for (;;) {
        struct ::statvfs status {};
        if (::statvfs(path.c_str(), &status) == 0) {
            std::uint32_t first = 0U;
            std::uint32_t second = 0U;
            std::memcpy(&first, &status.f_fsidx.__fsid_val[0], sizeof(first));
            std::memcpy(&second, &status.f_fsidx.__fsid_val[1], sizeof(second));
