#ifndef SYSCAPE_DETAIL_POWER_FREEBSD_HPP
#define SYSCAPE_DETAIL_POWER_FREEBSD_HPP
...
inline result<std::vector<power_common::battery_record>> batteries() {
    ...
    // FreeBSD hw.acpi.battery sysctls report aggregate status for all connected
    // units. If exactly 1 unit is present, label it BAT0; if multiple units are
    // aggregated, use an empty identifier representing the logical aggregate
    // view.
