#ifndef SYSCAPE_DETAIL_POWER_ANDROID_HPP
#define SYSCAPE_DETAIL_POWER_ANDROID_HPP
...
inline result<std::vector<power_common::battery_record>> batteries() {
    android::directory_handle dir("/sys/class/power_supply");
    if (!dir.valid()) {
    ...
    for (;;) {
        ...
        const auto type_content = android::read_text_file(type_path.c_str());
        if (!type_content) {
        ...
        android::strip_trailing_newlines(type_sv);
        if (type_sv != "Battery")
            continue;
        ...
        const auto pres_val =
            android::read_text_file((dir_path + "/present").c_str());
        if (pres_val) {
            std::string_view psv = *pres_val;
            android::strip_trailing_newlines(psv);
            if (psv == "1") {
                is_present = true;
            } else if (psv == "0") {
                is_present = false;
        ...
        const auto status_val =
            android::read_text_file((dir_path + "/status").c_str());
        if (status_val) {
        ...
        const auto cap_val =
            android::read_text_file((dir_path + "/capacity").c_str());
        if (cap_val) {
            std::uint32_t pct = 0U;
...
inline result<power_common::external_presence> external_power_online() {
    ...
    for (;;) {
        ...
        android::strip_trailing_newlines(type);
        if (type == "Mains" || type == "USB" || type == "Wireless") {
            const auto online_content =
                android::read_text_file((dir_path + "/online").c_str());
            if (online_content) {
