...
| GNU/Hurd | Primarily x86 and x86-64 | GCC, Clang where available | Hosted Full | Initial OS, CPU (logical processor count via sysconf, vendor ID and model name via /proc/cpuinfo, cumulative usage via /proc/stat), memory (physical, available, page size, swap status via /proc/meminfo), process (PID, PPID, working directory, executable path via /proc/self/exe, command line via /proc/self/cmdline, CPU times via times, thread count, priority, limits), process list (enumeration via /proc, real UID/GID, parent PID, command name), user (POSIX identity, privilege, utmpx sessions), filesystem (capacity space via statvfs, path limits via pathconf, volume ID, mount enumeration via mntent/procfs), network (interfaces via getifaddrs, DNS resolver via resolv.conf), environment, resource (load average via getloadavg, process count via /proc/loadavg, file descriptor limit via sysconf), and locale backends implemented; hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security return honest unsupported fallbacks; includes standalone unit tests for all 23 GNU/Hurd modules; registered in CI host checks with GCC and Clang (validating Hurd backend selection and host-testable POSIX paths; native GNU/Hurd toolchain verification pending dedicated Hurd runner environment) |
