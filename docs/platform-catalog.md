...
| FreeBSD | x86, x86-64, Arm, Arm64, RISC-V, PowerPC, and supported release architectures | Clang, GCC | Hosted Full | Initial OS, CPU, memory, process, user, filesystem, network interface, DNS configuration and statistics, environment, resource, locale, and IPC limits backends implemented; routes, gateways, process affinity, several CPU and memory refinements, system-wide scheduler/thread/handle totals, and IPC inventories return unsupported; implementation reviewed against current FreeBSD interfaces but not yet compiled or run on FreeBSD hardware or a FreeBSD VM |
