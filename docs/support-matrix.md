...
| 1 | `os.hpp` | Product name, version, build, kernel name and version, host name, boot time, uptime, and boot identifier where available | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 1 | `cpu.hpp` | Architecture, vendor, model, packages, physical and logical cores, topology, caches, instruction-set features, frequency, affinity, and utilization | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified. Cache geometry reports each platform's recorded values verbatim, while a backend that cannot identify distinct cache instances reports `not_supported`. Feature identifiers preserve each platform's own vocabulary instead of normalizing names across platforms. Affinity of the calling context belongs to the process module's scheduling slice rather than being duplicated here |
| 1 | `memory.hpp` | Physical memory, available memory, committed memory, swap or pagefile, page size, huge pages, pressure, and system utilization | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 1 | `process.hpp` | Current process identity, parent, executable, command line, working directory, start time, CPU time, memory use, priority, affinity, threads, and resource limits | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 1 | `user.hpp` | Current user identity, numeric or textual IDs, groups, home directory, shell, elevation, login session, active sessions, and logged-in users | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified. Fine-grained capability or per-privilege grants beyond the privilege classification remain not started |
| 1 | `filesystem.hpp` | Mounts, volumes, filesystem type, capacity, free space, block size, read-only state, and path limits | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 1 | `network.hpp` | Network interfaces, addresses, prefix lengths, MAC addresses, MTU, state, routes, default gateways, DNS configuration, and interface traffic and error statistics | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified. Interface enumeration with names, indices, operational state, loopback classification, link-layer addresses, MTU in bytes, unicast IPv4/IPv6 addresses with prefix lengths and numeric IPv6 scope identifiers, forwarding unicast routes, explicit default gateways, the system DNS resolver configuration with resolver addresses, ordered search domains where exposed, and separately recorded local domain name, as well as cumulative interface traffic and error statistics (rx/tx bytes, packets, errors, drops, multicast, and collisions) are complete. Host-name queries are provided by `os.hpp` rather than duplicated here |
| 1 | `locale.hpp` | Locale, preferred languages, country or region, text encoding, time zone, and UTC offset | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified. The current locale identifier, non-Unicode text-encoding label, and current UTC offset form the first slice; preferred languages, country or region, and time-zone identifiers form the second. Localized time-zone display names remain not started |
| 2 | `storage.hpp` | Physical drives, partitions, bus and media type, model, firmware, capacity, logical and physical sector sizes, rotational state, removable state, partition layout, and operating-system health / SMART diagnostics | Implemented; Linux whole-disk and partition enumeration verified; synthetic SMART parsers verified on Linux; real hardware NVMe/SCSI SMART ioctls unverified without administrative device node access; Windows verified on the recorded host; macOS implemented but unverified |
| 2 | `power.hpp` | Batteries, charge, health, charging state, energy and charge capacities, instantaneous electrical measurements, power source inventory, and estimated remaining time | Implemented; battery enumeration with verbatim labels, presence, operating condition, health condition, technology, manufacturer, model, serial, whole-percentage charge estimates, health degradation percentage, energy capacities (design, full, current in mWh), charge capacities (design, full, current in mAh), voltage (terminal and min design in mV), power rate in mW, temperature in °C, completed-cycle counts, power source enumeration, external-power-source detection, and runtime-to-empty estimate implemented where documented platform sources exist. Linux verified, Windows verified on the recorded host; macOS unverified |
| 2 | `hardware.hpp` | System manufacturer and model, chassis, motherboard, firmware or BIOS, hardware UUID, and documented device inventory | Implemented; system, motherboard, and firmware identity strings, SMBIOS chassis classification, firmware-recorded hardware UUID, and hardware device inventory (PCI/PCIe devices, USB bus devices, and SMBIOS physical memory modules/slots) are implemented where documented sources exist. Linux verified, Windows verified on the recorded host; macOS unverified |
| 2 | `display.hpp` | Displays, bounds, resolution, work area, refresh rate, scale, orientation, color depth, and connection state | Implemented; Linux connector and EDID enumeration verified; Windows desktop/display queries verified on the recorded host; macOS implementation unverified |
| 2 | `gpu.hpp` | GPU device enumeration, vendor classification, PCI IDs, model labels, driver names, VRAM capacity, and primary display adapter identification | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 2 | `virtualization.hpp` | Hypervisor presence and vendor, virtual machine hints, containers, namespaces, cgroups, jails, zones, WSL, and application sandboxing | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 2 | `environment.hpp` | Process environment snapshot, temporary and configuration directories, terminal presence, path resolution, and runtime environment characteristics | Implemented; environment-variable lookup, whole environment snapshot, standard user directories, current working directory, executable lookup in search path, path separators, and standard-stream terminal status implemented. Linux verified; Windows verified on the recorded host; macOS unverified |
| 2 | `resource.hpp` | System load, scheduler information, system-wide process and thread counts, open-file and open-handle totals, handle or file descriptor limits, and other capacity limits | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `security.hpp` | Secure Boot state, TPM presence, privilege state, filesystem encryption visibility, integrity facilities, Address Space Layout Randomization (ASLR), CPU hardware vulnerability mitigations, process capabilities, and security capabilities publicly exposed by the OS | Implemented; Secure Boot, TPM, Linux Security Module, Linux lockdown, ASLR configuration mode, CPU hardware vulnerability table, POSIX process capabilities, token privileges, and volume encryption visibility implemented. Linux verified; Windows verified on the recorded host; macOS unverified |
| 3 | `sensor.hpp` | Thermal zones, temperatures, fan speeds, and other sensors available through documented system interfaces | Implemented on Linux (verified); Windows and macOS stable unprivileged public OS APIs do not expose physical hardware temperatures or fan RPMs, and all queries honestly return `not_supported`; the Windows fallback is verified on the recorded host |
| 3 | `audio.hpp` | Audio input and output devices, default devices, capabilities, and connection state | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `input.hpp` | Keyboards, pointing devices, touch, game controllers, and other input devices exposed by the OS | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `camera.hpp` | Camera devices and non-invasive capabilities exposed without activating capture | Implemented; Linux verified, Windows 10 version 1803-or-later verified on the recorded host; macOS backend unverified. Default-camera selection is currently unsupported because the implemented backends do not expose an authoritative platform default |
| 3 | `printer.hpp` | Installed or discoverable printers, default printer, connection state, and basic capabilities | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `bluetooth.hpp` | Local adapters, radio state, capabilities, and paired devices when permission permits | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `wifi.hpp` | Wi-Fi adapters, radio state, current connection, signal information, and configured networks when permission permits | Implemented; Linux verified, Windows verified on the recorded host, macOS unsupported |
| 3 | `software.hpp` | OS-managed services, drivers, updates, installed applications or packages, and runtime components where a stable system source exists | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `process_list.hpp` | Other processes and their observable metadata, subject to permissions and platform privacy policy | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `connection.hpp` | Local listening endpoints and network connections visible to the current process or user | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `numa.hpp` | Non-Uniform Memory Access (NUMA) node topology, CPU core-to-node mapping, per-node memory, inter-node distance matrix, and thread node location | Implemented; Linux verified, Windows verified on the recorded host; macOS unverified |
| 3 | `ipc.hpp` | POSIX and System V shared memory, message queues, semaphore sets, local UNIX domain sockets, and system IPC limits | Implemented on Linux (verified) and for macOS system IPC limits via `sysctl` (unverified); macOS local socket inventory and every Windows query return `not_supported`; the Windows fallback is verified on the recorded host |
...
The macOS status must not advance until its headers compile with the official
SDK and the tests execute on a real macOS system.
...
The macOS status must not advance until its headers compile with the official SDK
and the tests execute on a real macOS system.
...
on the recorded host. The macOS status must not advance until its headers compile
with the official SDK and the tests execute on a real macOS system.
...
| Linux | In-process zero-dependency inspection of `/proc/modules` (driver name, memory footprint in bytes, reference counts, operational states), systemd unit directories (`/etc/systemd/system`, `/run/systemd/system`, `/usr/lib/systemd/system`, `/lib/systemd/system`) with `.wants`/`.requires` enable discovery and `/dev/null` mask detection, package manager catalogs (`/var/lib/pacman/local/` for Arch Linux, `/var/lib/dpkg/status` for Debian/Ubuntu, `/lib/apk/db/installed` for Alpine Linux), freedesktop `.desktop` files under `/usr/share/applications/`, `/usr/local/share/applications/`, and `~/.local/share/applications/`, update sources (`/run/reboot-required.pkgs` and `/run/reboot-required` for packages pending system restart, plus PackageKit `/var/lib/PackageKit/prepared-update` key files and their legacy line-oriented package IDs), and installed runtimes with executable binary and version-directory validation across JVMs (`/usr/lib/jvm/`), .NET (`/usr/share/dotnet/`, `~/.dotnet/`), Python (`/usr/lib/python3.*`), Node.js (`~/.nvm/versions/node/`, headers), Rust (`~/.rustup/toolchains/` with `multirust-channel-manifest.toml` semantic version extraction), Go (the version-bearing first line of `/usr/lib/go/VERSION`), Ruby, and PHP. PackageKit package IDs require the documented four fields, malformed or empty prepared-update data is rejected, and public update and runtime text is validated as UTF-8. Missing update sources accurately return `not_supported`. No external commands or subprocesses are invoked. Entries sort deterministically | Arch Linux, Linux 7.1.9, x86-64, glibc 2.44. Strict C++17 GCC 16.2.1 and Clang 22.1.8 with `-pedantic-errors`, high-signal warnings, and `-Werror`; live driver enumeration, live service enumeration, live pacman package query, live system updates, live runtime discovery, synthetic procfs/systemd/dpkg/apk/desktop/reboot/PackageKit/runtime parsers including valid comma-separated package IDs, empty optional fields, missing fields, extra fields, and empty prepared-update data, forced generic fallback, repeated inclusion, two-translation-unit ODR, AddressSanitizer, and UndefinedBehaviorSanitizer tests passed | Verified for this module on the listed host |
