...
| DOS, FreeDOS, OS/2, ArcaOS, and other legacy environments | DOS and FreeDOS under the RTOS/Constrained profile; OS/2 and ArcaOS under the Hosted Full profile; AmigaOS, RISC OS, OpenVMS, and z/OS compile-time detection under Freestanding Minimal | Implemented across all 29 query modules and freestanding foundation: compile-time detection for DOS, OS/2, AmigaOS, RISC OS, OpenVMS, and z/OS via `target_operating_system()` and `target_execution_environment()`; active query backends implemented for DOS/FreeDOS and OS/2/ArcaOS across core domains (OS product/kernel/version/uptime, CPU processor counts, memory sizes and load percent, process ID/working directory, filesystem space and path limits, resource limits), with honest `not_supported` fallbacks across all remaining 23 domains; verified with GCC and Clang test suites (100% passing tests including detection, functional, ODR, and standalone header checks); physical vintage hardware verification remains pending |
