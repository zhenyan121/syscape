...
| Tizen | Arm, Arm64, and x86-family depending on product | Linux-derived sandboxed environment, POSIX sysconf and pathconf APIs, and configurable test/platform overrides | Implemented under the Sandboxed/Restricted profile: compile-time identification established via `SYSCAPE_TARGET_TIZEN` or SDK defines (reporting `operating_system::tizen` and `execution_environment::sandboxed`); active query backends implemented for 6 core domains (OS product 'Tizen' and kernel 'Linux', CPU counts, dynamic page size via `sysconf`, memory load percent computed from physical and available RAM snapshots, process identity and limits, and filesystem path limits via `pathconf`), with validated test/platform overrides (including CPU counts, RAM, PID/PPID, process priority, thread counts, uptime, and filesystem `NAME_MAX`/`PATH_MAX` limits); honest `not_supported` fallbacks across the remaining 23 domains; verified via host-side mock and compilation suites across GCC and Clang; real device verification pending |
| Sailfish OS | Arm and x86-family depending on product | Linux-derived Sailfish Core / Mer middleware, Sailjail application sandbox, POSIX sysconf and pathconf APIs, and configurable test/platform overrides | Implemented under the Sandboxed/Restricted profile (targeting Sailjail application sandbox restrictions; native unconfined processes follow Linux permissions): compile-time identification established via `SYSCAPE_TARGET_SAILFISH` or SDK defines (reporting `operating_system::sailfishos` and `execution_environment::sandboxed`); active query backends implemented for 6 core domains (OS product 'Sailfish OS' and kernel 'Linux', CPU counts, dynamic page size via `sysconf`, memory load percent computed from physical and available RAM snapshots, process identity and limits, and filesystem path limits via `pathconf`), with validated test/platform overrides (including CPU counts, RAM, PID/PPID, process priority, thread counts, uptime, and filesystem `NAME_MAX`/`PATH_MAX` limits); honest `not_supported` fallbacks across the remaining 23 domains; verified via host-side mock and compilation suites across GCC and Clang; real device verification pending |
| KaiOS | Product-dependent Arm targets using a web-oriented Linux-derived stack | Linux-derived mobile runtime, POSIX sysconf and pathconf APIs, and configurable test/platform overrides | Implemented under the Sandboxed/Restricted profile: compile-time identification established via `SYSCAPE_TARGET_KAIOS` or SDK defines (reporting `operating_system::kaios` and `execution_environment::sandboxed`); active query backends implemented for 6 core domains (OS product 'KaiOS' and kernel 'Linux', CPU counts, dynamic page size via `sysconf`, memory load percent computed from physical and available RAM snapshots, process identity and limits, and filesystem path limits via `pathconf`), with validated test/platform overrides (including CPU counts, RAM, PID/PPID, process priority, thread counts, uptime, and filesystem `NAME_MAX`/`PATH_MAX` limits); honest `not_supported` fallbacks across the remaining 23 domains; verified via host-side mock and compilation suites across GCC and Clang; real device verification pending |
...
| AIX | Hosted C++17 environments using POSIX and documented AIX interfaces | Implemented across core OS, CPU (logical processor count via sysconf, vendor ID, usage ticks via perfstat_cpu_total; physical cores, package count, clock frequencies, caches, and ISAs return unsupported), memory (physical, available, page size, swap status via perfstat_memory_total, load percent), process (pid, ppid, working directory, thread count, CPU times, memory usage, limits; executable path and command line return unsupported), process list (enumeration via getprocs64, real UID/GID, parent PID), user (POSIX identity, privilege, utmpx sessions), filesystem (capacity space via statvfs, path limits via pathconf, volume ID, mount enumeration via mntctl), network (interfaces via getifaddrs, statistics via libperfstat, DNS resolver via resolv.conf; routes and gateways return unsupported), environment, resource (load average via getloadavg, process count via perfstat_process, file descriptor limit), locale, and hardware (system manufacturer; model and device inventories return unsupported) modules; virtualization, audio, printer, NUMA, connection, software, storage, security, sensor, and power return unsupported fallbacks; includes standalone tests for all 23 AIX modules; registered in CI host checks with GCC and Clang |
| HP-UX | Hosted C++17 environments using POSIX and documented HP-UX interfaces (pstat, mntent); `_XOPEN_SOURCE_EXTENDED=1`, `_PSTAT64=1`, and `_ICOD_BASE_INFO=1` must be translation-unit compiler definitions and are propagated by the CMake and pkg-config integrations | Implemented across core OS (product name, product version via uname, kernel name and version via uname, host name and boot time via utmpx; uptime, build identifier, and boot identifier return unsupported), CPU (logical processor count via sysconf, cumulative usage ticks from one pstat_getprocessor snapshot filtered to enabled processors; vendor identifiers, model names, physical core count, package count, clock frequencies, caches, and ISAs return unsupported), memory (physical, available, page size, swap status accumulating 1 KiB block swap and SW_FS filesystem swap via pstat_getswap with {0, 0} when unconfigured, load percent via memory_common::utilization_percent), process (pid, ppid, working directory, CPU times, resident memory safely summed from all pstat resident segments, virtual memory safely summed from pst_vtsize/pst_vdsize/pst_vssize/pst_vshmsize/pst_vmmsize/pst_vusize/pst_viosize/pst_vrsesize, limits; thread count, executable path, and command line return unsupported), process list (enumeration via pstat_getproc including PID 0 with native error propagation, short name via pst_ucomm, real UID/GID, parent PID, safe page addition with dynamic page size, thread count returns nullopt), user (POSIX identity, privilege, utmpx sessions), filesystem (capacity space via statvfs, path limits via pathconf, volume ID, mount enumeration via thread-safe mntent/mnttab), network (interfaces via getifaddrs, bounded DNS resolver reads via resolv.conf; routes, statistics, and gateways return unsupported), environment, resource (load average via pstat_getdynamic, process count via pstat_getdynamic psd_activeprocs, file descriptor limit via sysconf), and locale modules; hardware identity and inventory, virtualization, audio, printer, NUMA, connection, software, storage, security, sensor, and power return unsupported fallbacks; includes standalone tests for all 23 HP-UX modules; registered in CI compatibility compile checks with `_PSTAT64` mock HP-UX 11i pstat interfaces under GCC and Clang; native HP-UX hardware verification pending |
| Haiku | Hosted C++17 applications using POSIX and public Haiku APIs | Implemented across core OS, CPU (logical count via get_cpu_info, topology core and package counts via get_cpu_topology_info, clock frequency via cpu_info), memory (physical, available, page size, load percent), process (pid, ppid, executable path via image_info, thread count, CPU times, memory usage, limits), process list (enumeration via get_next_team_info, real UID/GID, parent PID), user (POSIX identity and privilege), filesystem (capacity space, path limits, volume ID via dev_t and root ino_t), network (interfaces via getifaddrs, DNS resolver via resolv.conf), environment, resource (process/thread counts and file descriptor limit; load average is unsupported on r1beta5), locale, and virtualization (CPUID hypervisor detection) modules; process command line, filesystem mounts, hardware, audio, printer, NUMA, connection, software, storage, security, sensor, and power return unsupported fallbacks; includes standalone tests for all 23 Haiku modules; registered in CI matrix via Haiku VM (r1beta5) |
| Fuchsia | Hosted C++17 components using POSIX and public Fuchsia SDK interfaces under the Sandboxed/Restricted profile | Initial support across all 29 hosted query modules under the Sandboxed/Restricted profile: OS identity (`uname`, `gethostname`) and monotonic uptime/boot time (`CLOCK_MONOTONIC`); CPU logical processor count (`sysconf(_SC_NPROCESSORS_ONLN)`); memory page size (`sysconf(_SC_PAGESIZE)`); process identity, working directory, CPU times (`times()`), and resource limits (`getrlimit()`); POSIX numeric credentials (`getuid`, `geteuid`, `getgid`, `getegid`) and single-user identity (sessions and login name report `not_supported` as `<utmpx.h>` is unavailable); filesystem capacity, path limits, and volume ID via `statvfs`/`pathconf` (mount enumeration is unsupported in component namespaces); process file descriptor limit via `sysconf`; environment; and locale queries. Domains without verified component-accessible interfaces return honest `not_supported`, including physical/available memory and swap, CPU vendor/model/caches/frequencies/ticks, process list enumeration, system load average, network interfaces/routes/DNS, and peripheral domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side backend-selection tests cover all 29 modules and header isolation tests; registered in CI matrix host checks with GCC and Clang; native Fuchsia target verification pending |
| GNU/Hurd | Hosted C++17 environments using GNU Mach, POSIX, glibc, and procfs interfaces | Implemented across all 23 hosted query modules: core OS (product name, kernel name, kernel version, host name, uptime/boot time via sysinfo/procfs), CPU (logical processor count via sysconf, vendor ID and model name via /proc/cpuinfo, cumulative usage via /proc/stat), memory (physical capacity, available memory, page size, swap status via /proc/meminfo, load percent), process (PID, PPID, working directory, executable path via /proc/self/exe, command line via /proc/self/cmdline, CPU times via times, thread count, priority, limits), process list (enumeration via /proc, real UID/GID, parent PID, command name), user (POSIX identity, privilege, utmpx sessions), filesystem (capacity space via statvfs, path limits via pathconf, volume ID, mount enumeration via mntent/procfs), network (interfaces via getifaddrs, DNS resolver via resolv.conf), environment, resource (load average via getloadavg, process count via /proc/loadavg, file descriptor limit via sysconf), and locale modules; hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security return honest unsupported fallbacks; includes standalone unit tests for all 23 GNU/Hurd modules; registered in CI host checks with GCC and Clang (validating Hurd backend selection and host-testable POSIX paths; native GNU/Hurd toolchain verification pending dedicated Hurd runner environment) |
| SerenityOS | Hosted C++17 environments using documented SerenityOS LibC and procfs interfaces | Initial support across all 23 hosted query modules: OS identity and monotonic uptime/approximate boot time; memory page size; process identity, executable path, working directory, CPU times, priority and limits; PID enumeration with UID/GID ownership; POSIX user identity excluding sessions; filesystem capacity, path limits and volume ID excluding mount enumeration; network interfaces and DNS configuration; environment; process file descriptor limit; and locale queries. Queries without a verified source matching the portable semantics return not_supported, including CPU information, physical/available memory and swap, process command line/memory usage/name lookup, system load/resource counts, and the hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security domains. Host-side backend-selection tests cover all 23 modules; native SerenityOS compilation and real-target verification remain pending |
| Redox OS | Public hosted interfaces available to the system | Initial support across all 23 hosted query modules: OS identity and monotonic uptime/approximate boot time; CPU online logical processor count via /scheme/sys/cpu; memory page size via getpagesize; process identity, working directory, and priority; PID enumeration with execution state via /scheme/sys/context (thread context names and real UID/GID ownership do not provide verified process-level names or credentials, leaving name/uid/gid/user_name unpopulated); POSIX user identity (login/session queries return not_supported as getlogin_r and utmpx are unavailable on Redox); filesystem capacity, path limits, and volume ID; network DNS configuration via /etc/net/dns (with /etc/resolv.conf fallback); environment; and locale queries. Queries without a verified source matching the portable semantics return not_supported, including executable path (/scheme/sys/exe exposes debug names rather than absolute executable paths), CPU times (relibc times() panics), process resource limits (relibc getrlimit() stubs RLIM_INFINITY), process file descriptor limit (getdtablesize() returns -1), network interfaces (getifaddrs() returns ENOSYS), physical/available memory and swap, process command line/memory usage/name lookup, system load/resource counts, and the hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security domains. Host-side backend-selection tests cover all 23 modules; registered in CI matrix host checks with GCC and Clang; native Redox OS compilation and real-target verification remain pending |
| Cygwin, MinGW, MSYS2, WSL, and Wine | Backend selected by the produced executable's actual runtime, with compatibility-environment detection | Implemented across all 29 hosted query modules and freestanding foundation: compile-time detection identifies MinGW, MSYS2, and Cygwin via `syscape::target_compatibility_environment()` and `syscape::target_execution_environment()` (reporting `compatibility`); Cygwin provides active POSIX query implementations for OS, CPU, memory, process, filesystem, network, resource, and locale modules, with honest `not_supported` fallbacks for unsupported peripheral and subsystem domains; MinGW and MSYS2 use the Windows Hosted Full backend; Wine compatibility layer runtime detection is exposed via `syscape::virtualization::is_wine()`, `wine_version()`, and `wine_build_id()`; WSL 1 and WSL 2 runtime detection is exposed via `syscape::virtualization::is_wsl()` and `wsl_version()`; ChromeOS and SteamOS product variants are detected via Linux OS and hardware backends; tested with 139/139 ctest passing on GCC 16.2.1 and Clang 22.1.8 |
| WebAssembly (WASI and Emscripten) | WASI runtimes and Emscripten browser/Node.js toolchains; most host information is expected to be sandboxed or unavailable | Initial support under the Sandboxed/Restricted profile across all 23 hosted query modules for WASI and all 29 hosted query modules for Emscripten (including peripheral modules GPU, display, input, camera, Bluetooth, and Wi-Fi returning portable not_supported): runtime OS product, kernel identity, version, and uptime queries report `not_supported` because WASI and Emscripten are system interfaces and toolchain compatibility layers rather than independent host operating systems (compile-target identity is provided by `target_operating_system()`); memory page size queries `sysconf(_SC_PAGESIZE)` and reports `not_supported` if unavailable; CPU topology queries report `not_supported`; system-wide resource limits report `not_supported`; environment queries snapshot `environ`, verify interactive terminal descriptors via `isatty()`, and check verified write and file-creation capabilities for temporary directories and verified directory existence for home directories without swallowing configured errors, while executable discovery honestly reports `not_supported` without bypassing public input validation; locale queries dynamically inspect `setlocale(LC_CTYPE, nullptr)` and `nl_langinfo(CODESET)`. Domains without an honest sandboxed source report portable `not_supported`, including multi-process, multi-user, and peripheral domains; includes standalone tests for all WASI and Emscripten modules; WASI retains GCC/Clang host-selection checks, while Emscripten CI performs WebAssembly compilation with `em++` and executes the Emscripten tests with Node.js; browser and real-host verification remain pending |

### Tier 4: RTOS, bare-metal, and legacy targets

| Platform group | Intended scope | Current status |
| --- | --- | --- |
| QNX Neutrino | Hosted Full or RTOS/Constrained profile using POSIX and documented QNX Neutrino interfaces | Initial support across all 23 hosted query modules: OS identity (`uname`, `gethostname`) and monotonic uptime/boot time (`CLOCK_MONOTONIC`); CPU logical processor count via `sysconf` (`_SC_NPROCESSORS_ONLN`); memory page size (`_SC_PAGESIZE`); process identity, working directory, CPU times via `times()`, priority, and limits; PID enumeration via `/proc` directory scanning (natural PID ordering; real UID/GID and parent PID remain unpopulated); POSIX user identity and privilege (sessions return `not_supported` as `<utmpx.h>` is unavailable); filesystem capacity, path limits, and volume ID via `statvfs`/`pathconf` (mount enumeration unsupported); network interfaces via NetBSD-derived `getifaddrs()` and DNS configuration via `/etc/resolv.conf`; environment; process file descriptor limit via `sysconf`; and locale queries with TZ parsing. Queries without a verified source matching the portable semantics return honest `not_supported`, including physical/available memory and load percent, system load average, swap, CPU vendor/model/caches/ticks, process executable path/command line/memory usage, network routes/gateways/statistics, system resource process counts, and the hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security domains. Host-side backend-selection tests cover all 23 modules; registered in CI matrix host checks with GCC and Clang; native QNX compilation and real-target verification remain pending |
| VxWorks | Hosted or constrained modules using documented native and POSIX APIs | Initial support across all 23 hosted query modules: Distinguishes Real-Time Process (RTP, `__RTP__`) and Kernel mode (`_WRS_KERNEL`); Kernel mode routes process, user, filesystem, and network interface queries to honest `not_supported`. In RTP mode: OS identity (`uname`, dynamically sized `gethostname`) and monotonic uptime/boot time (`CLOCK_MONOTONIC`); CPU logical processor count via `sysconf` (`_SC_NPROCESSORS_ONLN`); memory page size via `sysconf` (`_SC_PAGESIZE`, without un-version-gated `getpagesize()` fallback); process identity, working directory (caller-grown buffer without GNU `getcwd(nullptr, 0)`), CPU times via `times()`, task scheduling priority via `sched_getparam()` (0..255, with direction governed by `posixPriorityNumbering`), and limits; POSIX numeric credentials (`getuid`, `geteuid`, `getgid`, `getegid`) and privilege classification (account database queries `user_name`, `home_directory`, `shell`, as well as `supplementary_groups`, `sessions`, and `login_name` report `not_supported` without `<pwd.h>`, `/etc/passwd`, or `<utmpx.h>`); filesystem capacity, path limits, and volume ID via `statvfs`/`pathconf` (mount enumeration unsupported); network interfaces via IPNET `getifaddrs()` gated by capability macro `SYSCAPE_HAS_VXWORKS_GETIFADDRS` (disabled by default unless `INCLUDE_IPWRAP_GETIFADDRS` is detected or explicitly defined by the build system; enabled in CI test builds) with template-deduced `ioctl` argument adaptation and `sa_len`-bounded netmask decoding, and DNS configuration via `/etc/resolv.conf` (enforcing strict directive token counts); environment (temp directory validates absolute path and directory existence); process file descriptor limit via `sysconf`; and locale queries with `TIMEZONE` (validating paired 6-digit DST ranges with month/day/hour bounds, DST-aware fallback rejection, and UTF-8 encoding) and TZ parsing (UTC offset computed via broken-down calendar difference without `%z`, with early validation of ambient `TIMEZONE`; text encoding reports `not_supported` without `<langinfo.h>` or `nl_langinfo`). Queries without a verified source matching the portable semantics return honest `not_supported`, including process list enumeration (VxWorks RTP does not expose `/proc` or a userland process table), physical/available memory and load percent, system load average, swap, CPU vendor/model/caches/ticks, process executable path/command line/memory usage, network routes/gateways/statistics, system resource process counts, and the hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security domains. Host-side backend-selection tests cover all 23 RTP modules plus a dedicated `kernel_vxworks` test suite for `_WRS_KERNEL`; registered in CI matrix host checks with GCC and Clang; native VxWorks compilation and real-target verification remain pending |
| RTEMS | Hosted Full C++17 modules using interfaces present in the RTEMS base SDK | Initial support across all 23 hosted query modules: OS identity through `uname` and monotonic uptime/boot time through `CLOCK_MONOTONIC`; CPU logical processor count and memory page size through `sysconf`; numeric process identity and working directory; numeric user credentials and privilege classification; filesystem path limits and volume ID through `pathconf`, `statvfs`, and `stat`; DNS configuration through `/etc/resolv.conf`; process environment and working-directory queries; process file descriptor limit through `sysconf`; and locale queries with TZ parsing. Queries without a verified base-SDK source matching the portable semantics return honest `not_supported`, including process CPU time (RTEMS `times()` records uptime and current-thread time rather than portable process user/system time), priority and resource limits, account-database and login information, filesystem capacity snapshots (no portable RTEMS read-only `statvfs` flag), network interfaces supplied only by optional stacks, process list enumeration, physical/available memory and load percent, system load average, swap, CPU vendor/model/caches/ticks, process executable path/command line/memory usage, network routes/gateways/statistics, system resource process counts, user sessions, and the hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security domains. Host-side backend-selection tests cover all 23 modules with GCC and Clang; native RTEMS compilation and real-target verification remain pending |
| Zephyr | Hosted Full C++17 profile using documented Zephyr kernel and optional POSIX interfaces | Initial support under the Hosted Full profile with strict C++17: OS product name and kernel name report 'Zephyr'; product/kernel version from `uname.release`, build identifier from `uname.version`, and host name from `uname.nodename` when `CONFIG_POSIX_SINGLE_PROCESS` is enabled; monotonic uptime through POSIX `CLOCK_MONOTONIC` when `CONFIG_POSIX_TIMERS` is enabled (boot time reports `not_supported` as Zephyr lacks a reliable default wall-clock epoch); memory page size and resource file descriptor limit through POSIX `sysconf` when `CONFIG_POSIX_SINGLE_PROCESS` is enabled (supporting both function and macro `CONFIG_POSIX_SYSCONF_IMPL_MACRO` modes); CPU logical processor count through POSIX `sysconf(_SC_NPROCESSORS_ONLN)` when available. Because Zephyr is a single-address-space RTOS without multi-user or traditional Unix process/subsystem semantics, process, user, filesystem, network, locale, and environment queries honestly report `not_supported`, as do hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, security, and process list domains. Host-side backend-selection tests cover fallback and POSIX-enabled configurations across all 23 modules with GCC and Clang; native Zephyr compilation and real-target verification remain pending |
| NuttX | Hosted Full C++17 configurations using documented NuttX libc, scheduler, networking, and optional procfs interfaces | Initial support and host-side tests implemented across all 23 hosted query modules. Active queries cover OS identity and version through `uname`, monotonic uptime, logical processor count and page size through `sysconf`, RAM snapshots through `sysinfo`, current task PID/PPID, working directory, priority, affinity and the implemented open-file/stack limits, configuration-gated user identity and environment data, filesystem path limits, configuration-gated network interfaces and DNS nameservers, configuration-gated locale/UTC offset, file descriptor limits, and optional procfs task enumeration with name/state parsing. Boot time is unsupported because wall-clock initialization is optional; NuttX's synthetic root credentials without `CONFIG_SCHED_USER_IDENTITY`, zero-filled swap fields and unsupported `getrlimit` records, discarded `statvfs` mount flags, and instantaneous `sysinfo.loads` values are not exposed as corresponding portable facts. Remaining hardware, peripheral, security, storage, IPC, connection, and unsupported system queries return `not_supported`. Reviewed against Apache NuttX commit `078782846e2da63090d8d2fef7a2d1f21e20d357`; native NuttX compilation and real-target verification remain pending. |
| FreeRTOS | Hosted Full C++17 configurations using official FreeRTOS kernel APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name and kernel name report 'FreeRTOS'; version parsed from `tskKERNEL_VERSION_NUMBER`; monotonic uptime computed from `xTaskGetTickCount()` and `configTICK_RATE_HZ` or `portTICK_PERIOD_MS` with arithmetic overflow protection (boot time returns `not_supported` as FreeRTOS lacks a standard wall-clock epoch); CPU logical processor count discovered via `configNUM_CORES` (SMP FreeRTOS) with fallback to 1 (uniprocessor); memory total heap capacity via `configTOTAL_HEAP_SIZE` (reporting configured FreeRTOS managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `xPortGetFreeHeapSize()`, and load percent via `memory_load_percent`; process priority via `uxTaskPriorityGet(nullptr)`; system thread count via `uxTaskGetNumberOfTasks()`. Domains without standard FreeRTOS kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native FreeRTOS hardware target verification pending |
| ThreadX | Hosted Full C++17 configurations using official Eclipse ThreadX kernel APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'Eclipse ThreadX' and kernel name reports 'ThreadX'; version parsed from `_tx_version_id` and macros `THREADX_MAJOR_VERSION`/`THREADX_MINOR_VERSION`/`THREADX_PATCH_VERSION`; monotonic uptime computed from `tx_time_get()` and `TX_TIMER_TICKS_PER_SECOND` with arithmetic overflow protection (boot time returns `not_supported` as ThreadX lacks a standard wall-clock epoch); CPU logical processor count discovered via `TX_THREAD_SMP_MAX_CORES` (SMP ThreadX) with fallback to 1 (uniprocessor); task scheduling priority via `tx_thread_identify()` and `tx_thread_info_get()`; system thread count via `_tx_thread_created_count`. Domains without standard ThreadX kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size, heap/swap (ThreadX uses application-managed byte/block pools), user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native ThreadX hardware target verification pending |
| embOS | Hosted Full C++17 configurations using official SEGGER embOS APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'SEGGER embOS' and kernel name reports 'embOS'; version parsed from `OS_GetVersion()` (decoding major/minor/patch from version integer e.g. 51801 -> 5.18.1) or macro `OS_VERSION`; monotonic uptime computed from `OS_TIME_GetTicks()` and `OS_TICK_FREQ` or `OS_TIME_ConvertTicks2ms()` with 64-bit arithmetic overflow protection; CPU logical processor count discovered via `OS_CORE_GetNumCores()` (SMP embOS) with fallback to 1 (uniprocessor); memory total heap capacity via `OS_TOTAL_HEAP_SIZE` or `OS_HEAP_SIZE` (reporting configured embOS managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `OS_GetFreeHeapSpace()`, and load percent via `memory_load_percent`; task scheduling priority via `OS_TASK_GetPriority(nullptr)`; system thread count via `OS_TASK_GetNumTasks()`. Domains without standard embOS kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native embOS hardware target verification pending |
| uC/OS | Hosted Full C++17 configurations using official Micrium uC/OS APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'Micrium uC/OS-III' (or 'Micrium uC/OS-II') and kernel name reports 'uC/OS-III' (or 'uC/OS-II'); version parsed from `OSVersion()` or macro `OS_VERSION`; monotonic uptime computed from `OSTimeGet()` and `OS_CFG_TICK_RATE_HZ` or `OS_TICKS_PER_SEC` with 64-bit arithmetic overflow protection; CPU logical processor count discovered via `OS_CORE_NUM_CORES`, `OS_SMP_MAX_CORES`, or `LIB_DEF_MAX_CPU` (SMP uC/OS) with fallback to 1 (uniprocessor); memory total heap capacity via `OS_TOTAL_HEAP_SIZE` or `OS_HEAP_SIZE` (reporting configured uC/OS managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `OS_GetFreeHeapSpace()`, and load percent via `memory_load_percent`; task scheduling priority via `OSTCBCurPtr->Prio` (uC/OS-III) or `OSTCBCur->OSTCBPrio` (uC/OS-II); system thread count via `OSTaskQty` (uC/OS-III) or `OSTaskCtr` (uC/OS-II). Domains without standard uC/OS kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native uC/OS hardware target verification pending |
| INTEGRITY | Hosted Full C++17 configurations using official Green Hills INTEGRITY APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'Green Hills INTEGRITY' and kernel name reports 'INTEGRITY'; version parsed from `INTEGRITY_VERSION` or `INTEGRITY_MAJOR_VERSION`/`INTEGRITY_MINOR_VERSION`/`INTEGRITY_PATCH_VERSION`; monotonic uptime computed from `GetTime()` with 64-bit nanosecond-to-millisecond conversion and overflow protection (boot time returns `not_supported` as INTEGRITY lacks a standard wall-clock epoch); CPU logical processor count discovered via `GetProcessorCount()` (SMP INTEGRITY) with fallback to 1 (uniprocessor); memory total heap capacity via `INTEGRITY_TOTAL_HEAP_SIZE` or `INTEGRITY_HEAP_SIZE` (reporting configured INTEGRITY managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `GetMemoryPoolFreeSpace()`, and load percent via `memory_load_percent`; task scheduling priority via `CurrentTask()` and `GetPriority()`; system thread count via `INTEGRITY_TASK_COUNT` or `INTEGRITY_ACTIVE_TASKS`. Domains without standard INTEGRITY kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native INTEGRITY hardware target verification pending |
| T-Kernel | Hosted Full C++17 configurations using documented TRON Forum T-Kernel APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'T-Kernel' (or 'TRON µT-Kernel' for μT-Kernel) and kernel name reports 'T-Kernel'; version parsed from `tk_ref_ver(&rver)` or macros `TK_VERSION`/`TK_MAJOR_VERSION`/`TK_MINOR_VERSION`/`TK_PATCH_VERSION` / `UTK_MAJOR_VERSION`; monotonic uptime computed from `tk_get_tim()` with 64-bit nanosecond/millisecond arithmetic and overflow protection (boot time returns `not_supported` as T-Kernel lacks a standard wall-clock epoch); CPU logical processor count discovered via `TK_NUM_CORES`, `TK_CORE_COUNT`, or `TK_SMP_MAX_CORES` (SMP T-Kernel) with fallback to 1 (uniprocessor); memory total heap capacity via `TK_TOTAL_HEAP_SIZE`, `TK_HEAP_SIZE`, or `UTK_TOTAL_HEAP_SIZE` (reporting configured T-Kernel managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `tk_ref_mpl()` (`rmpl.frsz`), and load percent via `memory_load_percent`; task scheduling priority via `tk_ref_tsk(TSK_SELF, &rtsk)` (`rtsk.tskpri`); system thread count via `TK_TASK_COUNT`, `TK_ACTIVE_TASKS`, or `UTK_TASK_COUNT`. Domains without standard T-Kernel kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native T-Kernel hardware target verification pending |
| ChibiOS | Hosted Full C++17 configurations using official ChibiOS APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'ChibiOS/RT' (or 'ChibiOS/NIL' for ChibiOS/NIL) and kernel name reports 'ChibiOS/RT' (or 'ChibiOS/NIL'); version parsed from `CH_KERNEL_VERSION` or macros `CH_KERNEL_MAJOR`/`CH_KERNEL_MINOR`/`CH_KERNEL_PATCH` / `CH_VERSION`; monotonic uptime computed from `chVTGetSystemTimeX()` / `chVTGetSystemTime()` and `CH_CFG_ST_FREQUENCY`, `CH_ST_FREQUENCY`, or `CH_FREQUENCY` with 64-bit millisecond conversion and overflow protection (boot time returns `not_supported` as ChibiOS lacks a standard wall-clock epoch); CPU logical processor count discovered via `PORT_CORES_NUMBER`, `CH_PORT_CORES_NUMBER`, or `CH_NUM_CORES` (SMP ChibiOS) with fallback to 1 (uniprocessor); memory total heap capacity via `CH_TOTAL_HEAP_SIZE`, `CH_HEAP_SIZE`, `CH_CFG_MEMCORE_SIZE`, `CH_MEMCORE_SIZE`, or `CH_CORE_SIZE` (reporting configured ChibiOS managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `chHeapStatus()` (`*totalp`), and load percent via `memory_load_percent`; task scheduling priority via `chThdGetPriorityX()`; system thread count via `chRegFirstThread()` / `chRegNextThread()` traversal when `CH_CFG_USE_REGISTRY` or `CH_USE_REGISTRY` is enabled, or fallback macros `CH_CFG_MAX_THREADS` / `CH_TASK_COUNT` / `CH_CFG_NUM_THREADS` / `CH_NUM_THREADS`. Domains without standard ChibiOS kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native ChibiOS hardware target verification pending |
| Apache Mynewt | Hosted Full C++17 configurations using official Apache Mynewt APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'Apache Mynewt' and kernel name reports 'Mynewt OS'; version parsed from `MYNEWT_VERSION_STRING` or macros `MYNEWT_VERSION_MAJOR`/`MYNEWT_VERSION_MINOR`/`MYNEWT_VERSION_REVISION` / `OS_VERSION`; monotonic uptime computed from `os_time_get()` and `OS_TICKS_PER_SEC` or `MYNEWT_VAL(OS_TICKS_PER_SEC)` with 64-bit millisecond conversion and arithmetic overflow protection (boot time returns `not_supported` as Mynewt lacks a standard wall-clock epoch); CPU logical processor count discovered via `MYNEWT_VAL(OS_CORES)`, `MYNEWT_VAL(OS_NUM_CORES)`, or `MYNEWT_CORES` with fallback to 1 (uniprocessor); memory total heap capacity via `MYNEWT_TOTAL_HEAP_SIZE`, `MYNEWT_VAL(OS_HEAP_SIZE)`, `OS_TOTAL_HEAP_SIZE`, or `OS_HEAP_SIZE` (reporting configured Mynewt managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `MYNEWT_FREE_HEAP_SIZE`, `MYNEWT_VAL(OS_FREE_HEAP_SIZE)`, or `OS_FREE_HEAP_SIZE`, and load percent via `memory_load_percent`; task scheduling priority via `os_sched_get_current_task()->t_prio`; system thread count via `os_task_count()` or fallback macros `MYNEWT_TASK_COUNT` / `MYNEWT_MAX_TASKS` / `OS_TASK_COUNT` / `MYNEWT_VAL(OS_MAIN_TASK_PRIO)`. Domains without standard Mynewt kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native Apache Mynewt hardware target verification pending |
| Mbed OS | Hosted Full C++17 configurations using official Mbed OS and CMSIS-RTOS2 APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'Arm Mbed OS' and kernel name reports 'Mbed OS'; version parsed from `MBED_VERSION_STRING` or macros `MBED_MAJOR_VERSION`/`MBED_MINOR_VERSION`/`MBED_PATCH_VERSION` / `MBED_VERSION`; monotonic uptime computed from CMSIS-RTOS2 `osKernelGetTickCount()` and `MBED_CONF_RTOS_TICK_FREQ`, `OS_TICK_FREQ`, `MBED_TICK_FREQ`, or `osKernelGetTickFreq()` with 64-bit millisecond conversion and arithmetic overflow protection (boot time returns `not_supported` as Mbed OS lacks a standard wall-clock epoch); CPU logical processor count discovered via `MBED_CPU_COUNT`, `MBED_CONF_RTOS_CPU_COUNT`, or `NUM_CORES` with fallback to 1 (uniprocessor); memory total heap capacity via `mbed_stats_heap_t::reserved_size` (`mbed_stats_heap_get`), `MBED_TOTAL_HEAP_SIZE`, `MBED_HEAP_SIZE`, or `MBED_CONF_TARGET_DEFAULT_HEAP_SIZE` (reporting configured Mbed OS managed heap capacity rather than total hardware physical RAM/SRAM), available free heap via `mbed_stats_heap_t` (`reserved_size - current_size`) or `MBED_FREE_HEAP_SIZE`, and load percent via `memory_load_percent`; task scheduling priority via `osThreadGetPriority(osThreadGetId())`; system thread count via `osThreadGetCount()` or fallback macros `MBED_THREAD_COUNT` / `MBED_TASK_COUNT` / `MBED_MAX_THREADS`. Domains without standard Mbed OS kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native Mbed OS hardware target verification pending |
| RIOT | Hosted Full C++17 configurations using official RIOT OS APIs under the RTOS/Constrained profile | Initial support implemented across all 29 hosted query modules under the RTOS/Constrained profile: OS product name reports 'RIOT' and kernel name reports 'RIOT'; version parsed from `RIOT_VERSION` or `RIOT_VERSION_STRING` or `RIOT_VERSION_CODE`; monotonic uptime computed from `ztimer64_now(ZTIMER64_MSEC)`, `ztimer_now(ZTIMER_MSEC)`, or `xtimer_now_usec64()` with 64-bit millisecond conversion and arithmetic overflow protection (boot time returns `not_supported` as RIOT lacks a standard wall-clock epoch); CPU logical processor count discovered via `CONFIG_SMP_MAX_CORES`, `RIOT_CPU_COUNT`, or `NUM_CORES` with fallback to 1 (uniprocessor); memory physical RAM size via `cpu_get_ram_size()` when supported by target CPU/board facilities, available free heap via `get_mem_usage()` when the `malloc_monitor` module is enabled, and load percent via `memory_load_percent`; task scheduling priority via `thread_get_priority(thread_get(thread_getpid()))` (0 highest priority through `THREAD_PRIORITY_MIN` lowest priority); system thread count via `sched_num_threads` or fallback macros `RIOT_THREAD_COUNT` / `RIOT_MAX_THREADS`. Domains without standard RIOT kernel interfaces return honest `not_supported`, including process identity (PIDs), CPU frequencies/topology/caches/ticks, memory page size and swap, user identity/privilege, filesystem capacity/paths/mounts, network interfaces/routes/DNS, system load average and file descriptor limits, and peripheral/subsystem domains (hardware, power, storage, sensor, security, virtualization, audio, bluetooth, camera, connection, display, gpu, input, ipc, numa, printer, software, wifi). Host-side mock and backend-selection tests cover all 29 modules and header isolation targets; registered in CI matrix host checks with GCC and Clang; native RIOT hardware target verification pending |
| Other cataloged RTOS families | Selective runtime modules through official APIs and explicit board providers | Not started |
| Arduino, AVR, ESP, STM32, RP-series, Nordic, TI, NXP, Renesas, Microchip, RISC-V MCU, GD32, CH32, and BL-series families | Freestanding Minimal plus board-provider capabilities | Implemented under the Freestanding Minimal profile with strict C++11 and zero heap allocations: compile-time MCU and board family detection via `syscape::target_board_family()` across 16 families (Arduino, Microchip AVR, Microchip SAM, Espressif ESP, STMicroelectronics STM32, Raspberry Pi RP2040/RP2350, Nordic Semiconductor nRF, Texas Instruments MCU, NXP Semiconductors, Renesas Electronics, Microchip PIC, SiFive RISC-V, GigaDevice GD32, WCH CH32, Bouffalo Lab BL-series, and generic); compile-time execution environment detection mapping bare-metal MCU builds to `execution_environment::bare_metal`; explicit, allocation-free `syscape::board_provider` interface with function-pointer callbacks and global/static registration (`register_board_provider`, `current_board_provider`, `clear_board_provider`, and `SYSCAPE_STATIC_BOARD_PROVIDER`) for BSPs and firmware to supply CPU clock frequency, flash/SRAM/EEPROM capacities, GPIO pin counts, and supply voltage; in-process `board_info` snapshot query (`syscape::board::info()`); verified with GCC and Clang test suites under strict C++11, C++14, and C++17 modes including `-ffreestanding` and `-pedantic-errors`; hardware on-chip verification pending |
| DOS, FreeDOS, OS/2, ArcaOS, AmigaOS, RISC OS, OpenVMS, z/OS, IBM i, and other legacy environments | DOS, FreeDOS, AmigaOS, and RISC OS under the RTOS/Constrained profile; OS/2, ArcaOS, OpenVMS, z/OS, and IBM i under the Hosted Full profile | Implemented across all 29 query modules and freestanding foundation: compile-time detection for DOS, OS/2, AmigaOS, RISC OS, OpenVMS, z/OS, and IBM i via `target_operating_system()` and `target_execution_environment()`; active query backends implemented for DOS/FreeDOS, OS/2/ArcaOS, AmigaOS/MorphOS, RISC OS, OpenVMS, z/OS, and IBM i across core domains (compile-target OS product/kernel identity, CPU processor counts, filesystem path limits, and override configuration for version, uptime, memory, process identity, task priority, and resource counts), with honest `not_supported` fallbacks across all remaining 23 domains; verified with GCC and Clang test suites (100% passing tests including detection, functional, ODR, override, and standalone header checks); physical vintage hardware verification remains pending |
...
| Architecture, toolchain, and execution-environment detection | Arch Linux, Linux 7.1.8, x86-64, glibc 2.44 | GCC 16.2.1 with libstdc++ | Standalone headers, forced unknown target, forced generic backend, runtime assertions, synthetic toolchain version decoding suite (24 mock targets) | Verified |
| Architecture, toolchain, and execution-environment detection | Arch Linux, Linux 7.1.8, x86-64, glibc 2.44 | Clang 22.1.8 with libstdc++ | Standalone headers, forced unknown target, forced generic backend, runtime assertions, synthetic toolchain version decoding suite (24 mock targets) | Verified |
...
4. **BSD and mobile:** port core modules to BSD, Android, Apple mobile,
   HarmonyOS/OpenHarmony, Tizen, Sailfish OS, and KaiOS, respecting sandbox and
   permission limits.
