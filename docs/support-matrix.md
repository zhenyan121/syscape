...
| 1 | `user.hpp` | Current user identity, numeric or textual IDs, groups, home directory, shell, elevation, login session, active sessions, and logged-in users | Implemented; Linux verified, Windows and macOS unverified. Fine-grained capability or per-privilege grants beyond the privilege classification remain not started |
...
| 2 | `environment.hpp` | Process environment snapshot, temporary and configuration directories, terminal presence, path resolution, and runtime environment characteristics | Implemented; environment-variable lookup, whole environment snapshot, standard user directories, current working directory, executable lookup in search path, path separators, and standard-stream terminal status implemented. Linux verified; Windows and macOS unverified |
...
| 3 | `software.hpp` | OS-managed services, drivers, updates, installed applications or packages, and runtime components where a stable system source exists | Implemented; Linux verified, Windows and macOS unverified |
| 3 | `process_list.hpp` | Other processes and their observable metadata, subject to permissions and platform privacy policy | Implemented; Linux verified, Windows and macOS unverified |
| 3 | `connection.hpp` | Local listening endpoints and network connections visible to the current process or user | Implemented; Linux verified, Windows and macOS unverified |
| 3 | `numa.hpp` | Non-Uniform Memory Access (NUMA) node topology, CPU core-to-node mapping, per-node memory, inter-node distance matrix, and thread node location | Implemented; Linux verified, Windows and macOS unverified |
| 3 | `ipc.hpp` | POSIX and System V shared memory, message queues, semaphore sets, local UNIX domain sockets, and system IPC limits | Implemented on Linux (verified) and for macOS system IPC limits via `sysctl` (unverified); macOS local socket inventory and every Windows query return `not_supported` |
...
| Windows | Official Win32 IP Helper APIs in `iphlpapi.h`: `GetIfTable2` and `GetIfEntry2` (`MIB_IF_TABLE2` / `MIB_IF_ROW2`), reading `InOctets`, `OutOctets`, checked sums of the unicast and non-unicast packet counters, `InErrors`, `OutErrors`, `InDiscards`, and `OutDiscards`. `rx_multicast` remains unavailable because this API exposes only a combined non-unicast packet count and separate multicast octets, not a multicast packet count. Statistics names are joined by interface index to the names exposed by `interfaces()` when the adapter table is available; otherwise they use the UTF-8-converted `Alias` with `Description` fallback. Applications must link `Iphlpapi.lib` | Backend written from public Microsoft Win32 APIs with synthetic `MIB_IF_TABLE2` conversion, overflow, null-table, and error-contract tests; no Windows SDK or runtime available in the current environment | In progress; uncompiled and unverified |
| macOS | `PF_ROUTE` `NET_RT_IFLIST2` sysctl records (`if_msghdr2`) reading 64-bit `if_data64` counters `ifi_ibytes`, `ifi_obytes`, `ifi_ipackets`, `ifi_opackets`, `ifi_ierrors`, `ifi_oerrors`, `ifi_iqdrops`, `ifi_imcasts`, and `ifi_collisions`, plus `ifm_snd_drops` for outgoing queue drops | Backend written from public Apple Darwin routing-socket definitions with synthetic 64-bit message conversion, malformed-message, and lookup-error tests; no Apple SDK or runtime available in the current environment | In progress; uncompiled and unverified |
...
### Storage drive and partition enumeration evidence

The storage module exposes hardware-backed whole-disk drives and partitions
recorded by the platform, ordered by ascending identifier.
...
| Backend | Data sources and limitations | Evidence | State |
| --- | --- | --- | --- |
| Generic Hosted Full fallback | Portable `not_supported` results for every storage query (`drives()`, `partitions()`, and `disk_partitions()`) | Forced-backend standalone and runtime tests under GCC and Clang | Verified |
| Linux | The kernel-documented sysfs block ABI under `/sys/block`: `size` in fixed 512-byte sectors with explicit overflow rejection, nonzero `queue/logical_block_size` and `queue/physical_block_size`, `queue/rotational`, `removable`, and `device/{vendor,model,rev}` with firmware padding trimmed. Partitions under `/sys/block/<disk>/<part>` parsed via `partition`, `start`, `size`, `ro`, and `uevent` (`PARTN`, `PARTNAME`, `PARTUUID`). GPT vs MBR scheme classified from standard 36-char hyphenated UUIDs vs 8-hex-digit disk-signature format; correlated with `/proc/self/mounts` for mount points and filesystem types. Software constructs without backing hardware nodes are excluded | Arch Linux, Linux 7.1.8, x86-64, glibc 2.44. Strict C++17 GCC 16.2.1 and Clang 22.1.8 with `-pedantic-errors`, high-signal warnings, and `-Werror`; synthetic numeric overflow, flag, subsystem, UUID scheme classifier, partition boundary validation, and live query tests for drives, partitions, and disk_partitions cross-checked with sysfs; forced-fallback, repeated-inclusion, two-translation-unit ODR, AddressSanitizer, and UndefinedBehaviorSanitizer tests passed | Verified for this slice on the listed host |
| Windows | Present disks enumerated through SetupAPI disk device-interface class, `IOCTL_STORAGE_GET_DEVICE_NUMBER`, `IOCTL_STORAGE_QUERY_PROPERTY` for identity and block sizes, `IOCTL_DISK_GET_DRIVE_GEOMETRY_EX` for whole-disk capacity, and `IOCTL_DISK_GET_DRIVE_LAYOUT_EX` (`DRIVE_LAYOUT_INFORMATION_EX`, `PARTITION_INFORMATION_EX`) for partition tables (MBR/GPT styles, partition numbers, offsets, lengths, boot indicators, MBR types, GPT type/id GUIDs, GPT partition names, and read-only attributes). Volumes are enumerated independently through `FindFirstVolumeW`/`FindNextVolumeW`; `IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS` correlates them with partitions, `GetVolumePathNamesForVolumeNameW` supplies drive-letter and directory mount points, and `GetVolumeInformationW` supplies filesystem types. Variable-length extent buffers and UTF-16 text are validated before conversion. Unused container partitions are skipped | Backend written from public Microsoft APIs with synthetic descriptor, alignment, geometry, GUID formatting, UTF-16 name conversion, extent-buffer boundary validation, MBR/GPT layout conversion, volume correlation, and partition enumeration tests; no Windows SDK or runtime available in the current environment | In progress; uncompiled and unverified |
| macOS | IOKit `IOMedia` registry class and DiskArbitration framework (`DADiskCopyDescription` and `DADiskCopyWholeDisk`). Whole-disk media supply drive metadata and transport protocols. Partition candidates must explicitly record `kIOMediaWholeKey == FALSE`, resolve to a non-virtual whole disk accepted by the drive contract, and use that whole disk's BSD name as their parent. Partition media supply indices, base offsets, sizes, content-type renderings, read-only flags, volume names, CFUUID identifiers, and CFURL mount points. Content-type strings are not used to guess the parent partition-table scheme, so the scheme remains unknown when no documented table source is available | Backend written from public Apple interfaces with synthetic whole-media dictionary conversion, native CFUUID/CFURL conversion, partition dictionary conversion, parent-drive validation, and partition collection tests; no Apple SDK or runtime available in the current environment | In progress; uncompiled and unverified |
...
| Windows | CPUID leaf 1 ECX bit 31 and leaf 0x40000000 signature via `__cpuid`; `OpenProcessToken` and `GetTokenInformation(TokenIsAppContainer)` for sandbox detection; native Windows host reports `is_wsl() = false`; cgroup and namespace queries report `not_supported` | Backend written from public Microsoft APIs with synthetic CPUID register parsing and bare-metal validation; no Windows SDK or runtime available in the current environment | In progress; uncompiled and unverified |
...
| Linux | In-process zero-dependency inspection of `/proc/modules` (driver name, memory footprint in bytes, reference counts, operational states), systemd unit directories (`/etc/systemd/system`, `/run/systemd/system`, `/usr/lib/systemd/system`, `/lib/systemd/system`) with `.wants`/`.requires` enable discovery and `/dev/null` mask detection, package manager catalogs (`/var/lib/pacman/local/` for Arch Linux, `/var/lib/dpkg/status` for Debian/Ubuntu, `/lib/apk/db/installed` for Alpine Linux), freedesktop `.desktop` files under `/usr/share/applications/`, `/usr/local/share/applications/`, and `~/.local/share/applications/`, update indicators (`/run/reboot-required.pkgs` and `/run/reboot-required` for packages pending system restart), and installed runtimes with executable binary and version-directory validation across JVMs (`/usr/lib/jvm/`), .NET (`/usr/share/dotnet/`, `~/.dotnet/`), Python (`/usr/lib/python3.*`), Node.js (`~/.nvm/versions/node/`, headers), Rust (`~/.rustup/toolchains/` with `multirust-channel-manifest.toml` semantic version extraction), Go (the version-bearing first line of `/usr/lib/go/VERSION`), Ruby, and PHP. Public update and runtime text is validated as UTF-8. Missing update sources accurately return `not_supported`. No external commands or subprocesses invoked. Entries sort deterministically | Arch Linux, Linux 7.1.9, x86-64, glibc 2.44. Strict C++17 GCC 16.2.1 and Clang 22.1.8 with `-pedantic-errors`, high-signal warnings, and `-Werror`; live driver enumeration, live service enumeration, live pacman package query, live system updates, live runtime discovery, synthetic procfs/systemd/dpkg/apk/desktop/reboot/runtime parsers, forced generic fallback, repeated inclusion, two-translation-unit ODR, AddressSanitizer, and UndefinedBehaviorSanitizer tests passed | Verified for this module on the listed host |
| Windows | Win32 Service Control Manager (`OpenSCManagerW`, `EnumServicesStatusExW` with `SERVICE_WIN32` for services with `lpResumeHandle` pagination, `QueryServiceConfigW` for startup types and binary paths), `EnumDeviceDrivers` via `psapi.h` for actively loaded kernel drivers, Windows Uninstall registry enumeration with dynamic key length querying via `RegQueryInfoKeyW` under `HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall`, `HKLM\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall`, and `HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall`, Component Based Servicing updates (`HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Component Based Servicing\Packages` filtered for `0x60` Install Pending, plus `RebootPending` and `RebootRequired` indicators), and runtime discovery for .NET, Python, Java, and Node.js with NUL-validated dynamic registry strings, explicit 32-bit and 64-bit registry-view enumeration, and native error propagation. Applications must link `Advapi32.lib` and `Psapi.lib` | Backend written from public Microsoft Win32 APIs; no Windows SDK or runtime available in the current environment | In progress; uncompiled and unverified |
