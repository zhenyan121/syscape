...
| 2 | `storage.hpp` | Physical drives, partitions, bus and media type, model, firmware, capacity, logical and physical sector sizes, rotational state, removable state, partition layout, and operating-system health / SMART diagnostics | In progress; Linux whole-disk and partition enumeration verified; health telemetry and synthetic SMART parsers verified on Linux; real hardware SMART ioctls unverified without administrative device access; Windows and macOS uncompiled and unverified |
