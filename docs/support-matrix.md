...
| 3 | `ipc.hpp` | POSIX and System V shared memory, message queues, semaphore sets, local UNIX domain sockets, and system IPC limits | Implemented on Linux (verified); Windows and macOS backends are not started |
...
| Windows | Generic fallback returning `not_supported`; Win32 IPC enumeration has not been implemented | Placeholder backend header; no Windows SDK or runtime available in the current environment | Not started; uncompiled and unverified |
| macOS | Generic fallback returning `not_supported`; Mach and POSIX IPC enumeration has not been implemented | Placeholder backend header; no Apple SDK or runtime available in the current environment | Not started; uncompiled and unverified |
