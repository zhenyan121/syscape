...
| Tizen | Arm, Arm64, and x86-family depending on product | Linux-derived sandboxed environment, POSIX APIs, standard page size, and configurable test/platform overrides | Implemented across all 29 modules under the Sandboxed/Restricted profile: compile-time detection via `syscape::target_operating_system()` (reporting `operating_system::tizen`) and `target_execution_environment()` (reporting `execution_environment::sandboxed`); core query backends implemented for compile-target OS product ('Tizen') and kernel identity ('Linux'), standard page size (4096 bytes on ARM/x86), filesystem path limits (255-char component and 4096-char path limit), with configurable overrides for version, uptime, RAM sizes, memory load percent, CPU processor counts, PID/PPID/threads, task priority, and resource counts; honest `not_supported` fallbacks across remaining 23 domains; verified via host-side mock and compilation suites across GCC and Clang; real device verification pending |
| Sailfish OS | Arm and x86-family depending on product | Linux-derived Sailfish Core / Mer middleware, POSIX APIs, standard page size, and configurable test/platform overrides | Implemented across all 29 modules under the Sandboxed/Restricted profile: compile-time detection via `syscape::target_operating_system()` (reporting `operating_system::sailfishos`) and `target_execution_environment()` (reporting `execution_environment::sandboxed`); core query backends implemented for compile-target OS product ('Sailfish OS') and kernel identity ('Linux'), standard page size (4096 bytes on ARM/x86), filesystem path limits (255-char component and 4096-char path limit), with configurable overrides for version, uptime, RAM sizes, memory load percent, CPU processor counts, PID/PPID/threads, task priority, and resource counts; honest `not_supported` fallbacks across remaining 23 domains; verified via host-side mock and compilation suites across GCC and Clang; real device verification pending |
| KaiOS | Product-dependent Arm targets using a web-oriented Linux-derived stack | Linux-derived mobile runtime, POSIX APIs, standard page size, and configurable test/platform overrides | Implemented across all 29 modules under the Sandboxed/Restricted profile: compile-time detection via `syscape::target_operating_system()` (reporting `operating_system::kaios`) and `target_execution_environment()` (reporting `execution_environment::sandboxed`); core query backends implemented for compile-target OS product ('KaiOS') and kernel identity ('Linux'), standard page size (4096 bytes on ARM/x86), filesystem path limits (255-char component and 4096-char path limit), with configurable overrides for version, uptime, RAM sizes, memory load percent, CPU processor counts, PID/PPID/threads, task priority, and resource counts; honest `not_supported` fallbacks across remaining 23 domains; verified via host-side mock and compilation suites across GCC and Clang; real device verification pending |
