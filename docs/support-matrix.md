...
and explicit default gateways, as observed during each query. DNS
configuration and domain names remain not started; host-name queries belong
to `os.hpp`. Interface rows of families outside these slices are skipped
...
platform data rather than an unavailable-value sentinel. Route destinations
are canonical network addresses, on-link routes carry no next hop, and an
empty default-gateway list is valid. Metrics preserve the source platform's
raw value and are not comparable across operating systems.
