...
| 3 | `software.hpp` | OS-managed services, drivers, updates, installed applications or packages, and runtime components where a stable system source exists | In progress; system services, loaded kernel drivers, and installed software packages or desktop applications implemented. Linux verified, Windows and macOS unverified. System updates and runtime components remain planned |
...
### Software, service, driver, and package inventory evidence

This module exposes OS-managed system services and daemons, loaded kernel drivers and modules,
and installed software packages and applications: `services()`, `find_service()`, `loaded_drivers()`,
`find_driver()`, `installed_packages()`, and `find_package()`.
...
| Linux | In-process zero-dependency inspection of `/proc/modules` (driver name, memory footprint in bytes, reference counts, operational states), systemd unit directories (`/etc/systemd/system`, `/run/systemd/system`, `/usr/lib/systemd/system`, `/lib/systemd/system`) with `.wants`/`.requires` enable discovery and `/dev/null` mask detection, package manager catalogs (`/var/lib/pacman/local/` for Arch Linux, `/var/lib/dpkg/status` for Debian/Ubuntu, `/lib/apk/db/installed` for Alpine Linux), and freedesktop `.desktop` files under `/usr/share/applications/`, `/usr/local/share/applications/`, and `~/.local/share/applications/`. No external commands or subprocesses invoked. Entries sort deterministically by name | Arch Linux, Linux 7.1.9, x86-64, glibc 2.44. Strict C++17 GCC 16.2.1 and Clang 22.1.8 with `-pedantic-errors`, high-signal warnings, and `-Werror`; live driver enumeration, live service enumeration, live pacman package query, synthetic procfs/systemd/dpkg/apk/desktop entry parsers, forced generic fallback, repeated inclusion, two-translation-unit ODR, AddressSanitizer, and UndefinedBehaviorSanitizer tests passed | Verified for this module on the listed host |
| Windows | Win32 Service Control Manager (`OpenSCManagerW`, `EnumServicesStatusExW` with `SERVICE_WIN32` for services with `lpResumeHandle` pagination, `QueryServiceConfigW` for startup types and binary paths), `EnumDeviceDrivers` via `psapi.h` for actively loaded kernel drivers, and Windows Uninstall registry enumeration with dynamic key length querying via `RegQueryInfoKeyW` under `HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall`, `HKLM\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall`, and `HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall`. Applications must link `Advapi32.lib` and `Psapi.lib` | Backend written from public Microsoft Win32 APIs; no Windows SDK or runtime available in the current environment | In progress; uncompiled and unverified |
| macOS | LaunchDaemons and LaunchAgents plist parsing (`/System/Library/LaunchDaemons`, `/Library/LaunchDaemons`, `/System/Library/LaunchAgents`, `/Library/LaunchAgents`), and application bundle plist parsing via CoreFoundation (`/Applications/*.app/Contents/Info.plist`, `/System/Applications/*.app/Contents/Info.plist`, `/System/Applications/Utilities/*.app/Contents/Info.plist`, `~/Applications/*.app/Contents/Info.plist`). Loaded driver querying returns `not_supported` as macOS exposes no unprivileged public in-process API for loaded kernel extensions or DriverKit modules | Backend written from public Darwin file structures and CoreFoundation; no Apple SDK or runtime available in the current environment | In progress; uncompiled and unverified |
