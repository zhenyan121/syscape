...
| MINIX | Hosted C++17 environments using NetBSD-compatible userland, POSIX, and MINIX 3 kernel interfaces | Initial support across all 23 hosted query modules: OS identity (uname, gethostname) and monotonic uptime/boot time (CLOCK_MONOTONIC); CPU logical processor count via sysconf; memory page size via getpagesize; process identity, working directory, CPU times via times, priority, and limits; PID enumeration via /proc (real UID/GID and parent PID remain unpopulated); POSIX user identity and privilege, utmpx sessions; filesystem capacity, path limits, and volume ID (mount enumeration unsupported); network DNS configuration via /etc/resolv.conf; environment; process file descriptor limit via sysconf, and system load average via getloadavg; and locale queries. Queries without a verified source matching the portable semantics return not_supported, including physical/available memory and swap, CPU vendor/model/caches/ticks, process executable path/command line/memory usage, network interfaces/routes/statistics, system resource process counts, and the hardware, virtualization, audio, printer, NUMA, connection, software, storage, sensor, power, and security domains. Host-side backend-selection tests cover all 23 modules; registered in CI matrix host checks with GCC and Clang; native MINIX compilation and real-target verification remain pending |
