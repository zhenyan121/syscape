...
| Platform family | Initial variants | Intended platform sources | Current status |
| --- | --- | --- | --- |
| Android | Android NDK on Arm, Arm64, x86, and x86-64 | NDK APIs, Bionic system properties, procfs/sysfs available to applications, and POSIX APIs | Backend selection and standalone compile tests implemented across all 23 hosted modules. Active query backends implemented for core domains (OS, CPU, memory, process, process list, user, filesystem, network, environment, resource, power, storage, hardware, sensor, security, virtualization) using Android system properties (`__system_property_get`), safe procfs/sysfs access with SELinux EACCES handling, and POSIX facilities; restricted/unsupported domains (audio, connection, IPC, NUMA, printer, software) return portable `not_supported` fallbacks. Note: network MTU queries via AF_INET socket require `android.permission.INTERNET`. Compile-tested with GCC and Clang; physical on-device runtime verification pending. |
...
| DragonFly BSD | Supported releases and architectures available to the project | POSIX, `sysctl`, and documented DragonFly BSD interfaces | Implemented across core OS, CPU, memory, process, user, filesystem, network, environment, resource, locale, IPC limits, virtualization, hardware, power, sensor, security, NUMA, process list, storage, software, audio, printer, and connection modules; includes standalone tests for all 23 DragonFly BSD modules; source-reviewed against DragonFly BSD interfaces |
